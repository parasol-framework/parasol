/*
** Pseudo-random number generation.
** Copyright (C) 2005-2022 Mike Pall. See Copyright Notice in luajit.h
*/

#pragma once

#include "lj_def.h"

LJ_FUNC int lj_prng_seed_secure(PRNGState *rs);
LJ_FUNC uint64_t lj_prng_u64(PRNGState *rs);
LJ_FUNC uint64_t lj_prng_u64d(PRNGState *rs);

// This is just the precomputed result of lib_math.c:random_seed(rs, 0.0).
inline void lj_prng_seed_fixed(PRNGState *rs) noexcept
{
   rs->u[0] = U64x(a0d27757,0a345b8c);
   rs->u[1] = U64x(764a296c,5d4aa64f);
   rs->u[2] = U64x(51220704,070adeaa);
   rs->u[3] = U64x(2a2717b5,a7b7b927);
}
