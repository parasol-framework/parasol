...
{
   lj_assertX(not tvisint(key), "attempt to hash integer");
...
//
// The array size is non-inclusive. E.g. asize=128 creates array slots for 0..127, but not for 128. If you need
// slots 1..128, pass asize=129 (slot 0 is wasted in this case).
//
// The hash size is given in hash bits. hbits=0 means no hash part.  hbits=1 creates 2 hash slots, hbits=2 creates
// 4 hash slots and so on.
...
void lj_tab_resize(lua_State *L, GCtab *t, uint32_t asize, uint32_t hbits)
{
   ...
   if (asize < oldasize) {  // Array part shrinks?
      ...
      for (i = asize; i < oldasize; i++) { //  Reinsert old array values.
         if (not tvisnil(&array[i])) copyTV(L, lj_tab_setinth(L, t, (int32_t)i), &array[i]);
      }
   ...
   if (oldhmask > 0) {  // Reinsert pairs from old hash part.
      ...
      for (i = 0; i <= oldhmask; i++) {
         Node* n = &oldnode[i];
         if (not tvisnil(&n->val)) copyTV(L, lj_tab_set(L, t, &n->key), &n->val);
      }
      g = G(L);
...
{
   lj_assertX(not tvisint(key), "bad integer key");
   if (tvisnum(key)) {
...
static uint32_t bestasize(uint32_t bins[], uint32_t* narray)
{
   ...
   for (b = 0, sum = 0; 2 * nn > (1u << b) and sum != nn; b++)
      if (bins[b] > 0 and 2 * (sum += bins[b]) > (1u << b)) {
         sz = (2u << b) + 1;
         na = sum;
      }
   *narray = sz;
...
cTValue* lj_tab_get(lua_State* L, GCtab* t, cTValue* key)
{
   if (tvisstr(key)) {
      ...
      else goto genlookup;  //  Else use the generic lookup.
   }
   else if (not tvisnil(key)) {
      Node *n;
genlookup:
      n = hashkey(t, key);
      do {
...
TValue * lj_tab_newkey(lua_State *L, GCtab *t, cTValue *key)
{
   ...
   if (not tvisnil(&n->val) or t->hmask == 0) {
      ...
      } while (not tvisnil(&(--freenode)->key));

      setfreetop(t, nodebase, freenode);
      lj_assertL(freenode != &G(L)->nilnode, "store to fallback hash");
      collide = hashkey(t, &n->key);
      if (collide != n) {  // Colliding node not the main node?
         while (noderef(collide->next) != n) collide = nextnode(collide); //  Find predecessor.
         ...
         while (nextnode(freenode)) {
            ...
            if (not tvisnil(&nn->val) and hashkey(t, &nn->key) == n) {
               ...
               while ((nn = nextnode(freenode))) {
                  if (not tvisnil(&nn->val)) {
                     Node* mn = hashkey(t, &nn->key);
                     if (mn != freenode and mn != nn) {
                        freenode->next = nn->next;
                        nn->next = mn->next;
                        setmref(mn->next, nn);
                     }
...
TValue * lj_tab_set(lua_State *L, GCtab *t, cTValue *key)
{
   ...
   if (tvisstr(key)) return lj_tab_setstr(L, t, strV(key));
   else if (tvisint(key)) return lj_tab_setint(L, t, intV(key));
   else if (tvisnum(key)) {
      lua_Number nk = numV(key);
      int32_t k = lj_num2int(nk);
...
uint32_t lj_tab_keyindex(GCtab* t, cTValue* key)
{
   ...
   if (not tvisnil(key)) {
      Node* n = hashkey(t, key);
      do {
         if (lj_obj_equal(&n->key, key)) return t->asize + (uint32_t)((n + 1) - noderef(t->node));
      } while ((n = nextnode(n)));

      if (key->u32.hi == LJ_KEYINDEX) return key->u32.lo; //  Despecialized ITERN while running.
...
MSize lj_tab_len(GCtab* t)
{
   ...
   if (hi) {
      hi--;
      if (tvisnil(arrayslot(t, hi))) [[likely]] {
      ...
      else { // 0-based: hi is a valid index (since hi >= 0 is always true for size_t)
         last_index = hi;
      }
   }
...
MSize lj_tab_len_hint(GCtab *t, size_t hint)
{
   ...
   if (hint + 1 < asize) [[likely]] {
      if (not tvisnil(tv) and tvisnil(tv + 1)) return semantic_length(hint);
   }
   else if ((hint + 1 <= asize) and (t->hmask == 0) and (not tvisnil(tv))) {
