...
"Usage: tiri [options...] script.tiri [--arg1=v1 --arg2=v2 ...]\n\
\n\
Special options are:\n\
\n\
...
static int exec_source(CSTRING TargetFile, CSTRING Procedure)
{
   ...
   if (subclass != ID_TIRI) return -1;

   objScript *script;
   if (!NewObject(ID_TIRI, &script)) {
      ...
      for (i=0; args[i]; i++) {
         if (iequals(args[i], "--help")) {
            ...
            static CSTRING modules[] = { // These modules must be present for an installation to be valid.
               "audio", "display", "tiri", "font", "http", "json", "network", "picture", "svg", "vector", "xml"
            };
