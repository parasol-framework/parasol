...
LJLIB_ASM(string_rep) // PARASOL PATCHED IN: This function has been modified to accept nil strings in order to create empty string buffers of any size.
{
  GCstr *s = lj_lib_optstr(L, 1);
  int32_t k = lj_lib_checkint(L, 2);
  GCstr *sep = lj_lib_optstr(L, 3);
  int32_t len;
  if (s) len = (int32_t)s->len;
  else len = 0;
  global_State *g = G(L);
  int64_t tlen;
  const char *src;
  ...
  if (sep) {
    ...
    if (!len) tlen = (int64_t)k;
    else tlen = (int64_t)k * len;
    if (tlen > LJ_MAX_STR)
      lj_err_caller(L, LJ_ERR_STROV);
  }
