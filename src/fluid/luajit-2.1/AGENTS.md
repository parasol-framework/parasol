...
the script has ran.  For example:
...
local function sampleFunction(a, b)
   local scale = b * 2
   local total = a + scale
   return result
end
...
If a DebugLog string of `disasm,log` is used then the output will be printed to the debug log,
eliminating the need to print when logging is active.
