...
static AliasRet aa_ahref(jit_State* J, IRIns* refa, IRIns* refb)
{
   ...
   if (irref_isk(ka) and irref_isk(kb))
   ...
   if (refa->o == IR_AREF) {
      ...
      if (keya->o == IR_ADD and irref_isk(keya->op2)) {
         basea = keya->op1;
         ofsa = IR(keya->op2)->i;
         if (basea == kb and ofsa != 0)
            return ALIAS_NO;  //  t[base+-ofs] vs. t[base].
      }
      if (keyb->o == IR_ADD and irref_isk(keyb->op2)) {
         baseb = keyb->op1;
         ofsb = IR(keyb->op2)->i;
         if (ka == baseb and ofsb != 0)
            return ALIAS_NO;  //  t[base] vs. t[base+-ofs].
      }
      if (basea == baseb and ofsa != ofsb)
         return ALIAS_NO;  //  t[base+-o1] vs. t[base+-o2] and o1 != o2.
   }
   else {
      // Disambiguate hash references based on the type of their keys.
      lj_assertJ((refa->o == IR_HREF or refa->o == IR_HREFK or refa->o == IR_NEWREF) and
...
static TRef fwd_ahload(jit_State* J, IRRef xref)
{
   ...
   // No conflicting store (yet): const-fold loads from allocations.
   {
      ...
      IRRef tab = ir->op1;
      ir = IR(tab);
      if ((ir->o == IR_TNEW or (ir->o == IR_TDUP and irref_isk(xr->op2))) and
         ...
         */
         ...
         if (ir->o == IR_TNEW and !irt_isnil(fins->t))
         ...
         if (irt_ispri(fins->t)) {
            ...
            lj_assertJ(itype2irt(tv) == irt_type(fins->t), "mismatched type in constant table");
            if (irt_isnum(fins->t)) return lj_ir_knum_u64(J, tv->u64);
            else if (LJ_DUALNUM and irt_isint(fins->t)) return lj_ir_kint(J, intV(tv));
            else return lj_ir_kstr(J, strV(tv));
         }
...
static TRef fwd_aload_reassoc(jit_State* J)
{
   ...
   if (key->o == IR_ADD and irref_isk(key->op2)) {
      IRIns* add2 = IR(key->op1);
      if (add2->o == IR_ADD and irref_isk(add2->op2) and
         ...
         while (ref > lim) {
            IRIns* ir = IR(ref);
            if (ir->op1 == irx->op1 and ir->op2 == add2->op1)
...
TRef lj_opt_fwd_hrefk(jit_State* J)
{
   ...
   while (ref > tab) {
      IRIns* newref = IR(ref);
      if (tab == newref->op1) {
         if (fright->op1 == newref->op2 and fwd_aa_tab_clear(J, ref, tab))
   ...
   if (IR(tab)->o == IR_TDUP and fwd_aa_tab_clear(J, tab, tab))
...
int lj_opt_fwd_href_nokey(jit_State* J)
{
   ...
   if (irt_isnum(fright->t) and J->chain[IR_NEWREF] > lim) {
      ...
      if (IR(ref)->op1 == tab) {
         ...
         while (sref > ref) {
            ...
            if (tab == fref->op1) {  // ASTORE to the same table.
               ...
               if (!irt_isnil(store->t) and
                  idx->o == IR_ADD and idx->op1 == ref and
                  IR(idx->op2)->o == IR_KINT and IR(idx->op2)->i == 1) {
...
cselim:
   ...
   while (ref > lim) {
      IRIns* ir = IR(ref);
      if (ir->op1 == uref ||
         (IR(ir->op1)->op12 == IR(uref)->op12 and IR(ir->op1)->o == IR(uref)->o))
   ...
   while (ref > xref) {  // Search for redundant or conflicting stores.
      ...
      case ALIAS_MUST:   //  Store to the same location.
         ...
         if (ref > J->chain[IR_LOOP]) {  // Quick check to avoid crossing LOOP.
            ...
            *refp = store->prev;
            lj_ir_nop(store);
            if (ref + 1 < J->cur.nins and
               store[1].o == IR_OBAR and store[1].op1 == xref) {
...
TRef lj_opt_dse_fstore(jit_State* J)
{
   ...
   while (ref > fref) {  // Search for redundant or conflicting stores.
      ...
      case ALIAS_MAY:
         ...
         break;  //  Otherwise continue searching.
      case ALIAS_MUST:
         if (store->op2 == val and
            !(xr->op2 >= IRFL_SBUF_W and xr->op2 <= IRFL_SBUF_R))
...
int lj_opt_fwd_sbuf(jit_State* J, IRRef lim)
{
   ...
   while (ref > lim) {
      IRIns* ir = IR(ref);
      if (ir->op2 >= IRCALL_lj_strfmt_putint and ir->op2 < IRCALL_lj_buf_tostr)
         return 0;  //  Conflict.
      ref = ir->prev;
   }
...
static AliasRet aa_xref(jit_State* J, IRIns* refa, IRIns* xa, IRIns* xb)
{
   ...
   if (refa == refb and irt_sametype(xa->t, xb->t))
      return ALIAS_MUST;  //  Shortcut for same refs with identical type.
   // Offset-based disambiguation.
   if (refa->o == IR_ADD and irref_isk(refa->op2)) {
      IRIns* irk = IR(refa->op2);
      basea = IR(refa->op1);
      ofsa = (irk->o == IR_KINT64) ? (ptrdiff_t)ir_k64(irk)->u64 : (ptrdiff_t)irk->i;
   }
   if (refb->o == IR_ADD and irref_isk(refb->op2)) {
      IRIns* irk = IR(refb->op2);
      baseb = IR(refb->op1);
      ofsb = (irk->o == IR_KINT64) ? (ptrdiff_t)ir_k64(irk)->u64 : (ptrdiff_t)irk->i;
   }
   // Treat constified pointers like base vs. base+offset.
   if (basea->o == IR_KPTR and baseb->o == IR_KPTR) {
   ...
   */
   ...
   if (!irt_sametype(xa->t, xb->t) and
      !(irt_typerange(xa->t, IRT_I8, IRT_U64) and ((xa->t.irt - IRT_I8) ^ (xb->t.irt - IRT_I8)) == 1))
      return ALIAS_NO;
...
{
   ptrdiff_t ofs = 0;
   if (ir->o == IR_ADD and irref_isk(ir->op2)) {  // Get constant offset.
      IRIns* irk = IR(ir->op2);
      ofs = (LJ_64 and irk->o == IR_KINT64) ? (ptrdiff_t)ir_k64(irk)->u64 : (ptrdiff_t)irk->i;
      ir = IR(ir->op1);
   }
   if (ir->o == IR_ADD) {  // Add of base + index.
      ...
      if (ir1->o == IR_BSHL and irref_isk(ir1->op2))
         shift = IR(ir1->op2)->i;
      else if (ir1->o == IR_ADD and ir1->op1 == ir1->op2)
         shift = 1;
      else
         ir1 = ir;
      ir2 = IR(ir1->op1);
      // A non-reassociated add. Must be a loop-carried dependence.
      if (ir2->o == IR_ADD and irt_isint(ir2->t) and irref_isk(ir2->op2))
   ...
   while (ref > lim) {
      ...
      case ALIAS_MUST:
         ...
         if (!irt_sametype(fins->t, IR(store->op2)->t)) {
            ...
            if (dt == IRT_I8 or dt == IRT_I16) {  // Trunc + sign-extend.
               st = IRType(dt | IRCONV_SEXT);
               dt = IRT_INT;
            }
            else if (dt == IRT_U8 or dt == IRT_U16) {  // Trunc + zero-extend.
               st = dt;
               dt = IRT_INT;
            }
...
cselim:
   ...
   while (ref > lim) {
      ...
      if (IR(ref)->op1 == xref and irt_sametype(IR(ref)->t, fins->t))
         return ref;
      ref = IR(ref)->prev;
   }

   // Reassociate XLOAD across PHIs to handle a[i-1] forwarding case.
   if (!(fins->op2 & IRXLOAD_READONLY) and J->chain[IR_LOOP] and
      xref == fins->op1 and (xref = reassoc_xref(J, xr)) != 0) {
      ref = J->chain[IR_XSTORE];
