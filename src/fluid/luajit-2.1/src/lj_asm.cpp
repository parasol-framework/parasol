...
#define opisfusableload(o) \
  ((o) == IR_ALOAD or (o) == IR_HLOAD or (o) == IR_ULOAD or \
   (o) == IR_FLOAD or (o) == IR_XLOAD or (o) == IR_SLOAD or (o) == IR_VLOAD)
...
#define ra_used(ir)      (ra_hasreg((ir)->r) or ra_hasspill((ir)->s))
...
static Reg ra_rematk(ASMState* as, IRRef ref)
{
   ...
   if (ir->o == IR_KNUM) {
      ...
      if (emit_canremat(REF_BASE) and ir->o == IR_BASE) {
         ...
         lj_assertA(ir->o == IR_KINT or ir->o == IR_KGC ||
            ir->o == IR_KPTR or ir->o == IR_KKPTR or ir->o == IR_KNULL,
...
static Reg ra_evict(ASMState* as, RegSet allow)
{
   ...
   if (RID_NUM_FPR == 0 or allow < RID2RSET(RID_MAX_GPR)) {
      GPRDEF(MINCOST)
   }
   else {
      FPRDEF(MINCOST)
   ...
   lj_assertA(ra_iskref(ref) or (ref >= as->T->nk and ref < as->T->nins),
...
static void ra_evictk(ASMState* as)
{
   ...
   while (work) {
      ...
      if (emit_canremat(ref) and irref_isk(ref)) {
         ra_rematk(as, ref);
         checkmclim(as);
      }
      rset_clear(work, r);
   ...
   while (work) {
      ...
      if (ref < ASMREF_L) {
         if (ra_iskref(ref)) {
            ...
            IRIns* ir = IR(ref);
            if ((ir->o == IR_KINT64 and k == (int64_t)ir_kint64(ir)->u64) ||
#if LJ_GC64
            (ir->o == IR_KINT and k == ir->i) ||
               (ir->o == IR_KGC and k == (intptr_t)ir_kgc(ir)) ||
               ((ir->o == IR_KPTR or ir->o == IR_KKPTR) &&
                  k == (intptr_t)ir_kptr(ir))
#else
               (ir->o != IR_KINT64 and k == ir->i)
#endif
               )
               return r;
         }
...
static Reg ra_allocref(ASMState* as, IRRef ref, RegSet allow)
{
   ...
   if (pick) {
      ...
      if (ra_hashint(ir->r)) {
         ...
         if (rset_test(allow, r) and emit_canremat(regcost_ref(as->cost[r]))) {
      ...
      if (ref < as->loopref and !irt_isphi(ir->t)) {
         if ((pick & ~as->modset))
            pick &= ~as->modset;
         r = rset_pickbot(pick);  //  Reduce conflicts with inverse allocation.
      }
         ...
         if (RID_NUM_GPR > 8 and (pick & ~RSET_SCRATCH))
            pick &= ~RSET_SCRATCH;
         r = rset_picktop(pick);
      }
   }
...
static Reg ra_dest(ASMState* as, IRIns* ir, RegSet allow)
{
   ...
   if (ra_hasreg(dest)) {
      ...
      if (ra_hashint(dest) and rset_test((as->freeset & allow), ra_gethint(dest))) {
...
static void ra_left(ASMState* as, Reg dest, IRRef lref)
{
   ...
   if (ra_noreg(left)) {
      if (irref_isk(lref)) {
         if (ir->o == IR_KNUM) {
            ...
            if (tvispzero(ir_knum(ir)) or !(as->freeset & RSET_FPR)) {
      ...
      if (!ra_hashint(left) and !iscrossref(as, lref))
   ...
   if (dest != left) {
      ...
      if (irt_isphi(ir->t) and as->phireg[dest] == lref) {
         ra_modified(as, left);
...
static void ra_leftov(ASMState* as, Reg dest, IRRef lref)
{
   ...
   if (ra_noreg(left)) {
      ...
      left = ra_allocref(as, lref,
         (LJ_SOFTFP or dest < RID_MAX_GPR) ? RSET_GPR : RSET_FPR);
   }
   ra_noweak(as, left);
   if (dest != left) {
      // Use register renaming if dest is the PHI reg.
...
{
   if (irs->s == 255) {
      if (irs->o == IR_ASTORE or irs->o == IR_HSTORE ||
         irs->o == IR_FSTORE or irs->o == IR_XSTORE) {
         IRIns* irk = IR(irs->op1);
         if (irk->o == IR_AREF or irk->o == IR_HREFK)
            irk = IR(irk->op1);
...
{
   IRIns* ir = IR(ref);
   if (!irref_isk(ref) and ir->r != RID_SUNK) {
      ...
      if (ir->r == RID_SINK) {
         ...
         {  // Allocate stored values for TNEW, TDUP and CNEW.
            IRIns* irs;
            lj_assertA(ir->o == IR_TNEW or ir->o == IR_TDUP or ir->o == IR_CNEW,
               "sink of IR %04d has bad op %d", ref - REF_BIAS, ir->o);
            for (irs = IR(as->snapref - 1); irs > ir; irs--)
               if (irs->r == RID_SINK and asm_sunk_store(as, ir, irs)) {
                  lj_assertA(irs->o == IR_ASTORE or irs->o == IR_HSTORE ||
                     irs->o == IR_FSTORE or irs->o == IR_XSTORE,
         ...
         RegSet allow;
         if (ir->o == IR_CONV and ir->op2 == IRCONV_NUM_INT) {
            IRIns* irc;
            for (irc = IR(as->curins); irc > ir; irc--)
               if ((irc->op1 == ref or irc->op2 == ref) &&
                  !(irc->r == RID_SINK or irc->r == RID_SUNK))
                  goto nosink;  //  Don't sink conversion if result is used.
            asm_snap_alloc1(as, ir->op1);
            return;
         }
      nosink:
         allow = (!LJ_SOFTFP and irt_isfp(ir->t)) ? RSET_FPR : RSET_GPR;
         if ((as->freeset & allow) ||
            (allow == RSET_FPR and asm_snap_canremat(as))) {
...
static void asm_snap_alloc(ASMState* as, int snapno)
{
   ...
   for (n = 0; n < nent; n++) {
      ...
      if (!irref_isk(ref)) {
         ...
         if (LJ_SOFTFP and (sn & SNAP_SOFTFPNUM)) {
...
static void asm_gcstep(ASMState* as, IRIns* ir)
{
   ...
   for (ira = IR(as->stopins + 1); ira < ir; ira++)
      if ((ira->o == IR_TNEW or ira->o == IR_TDUP ||
...
static void asm_bufhdr(ASMState* as, IRIns* ir)
{
   ...
   case IRBUFHDR_APPEND: {
      ...
      IRIns* irp = IR(ir->op1);
      if (!(ra_hasreg(irp->r) or irp == ir - 1 ||
         (irp == ir - 2 and !ra_used(ir - 1)))) {
         while (!(irp->o == IR_BUFHDR and irp->op2 == IRBUFHDR_RESET))
            irp = IR(irp->op1);
...
static void asm_bufput(ASMState* as, IRIns* ir)
{
   ...
   else if (mayfuse(as, ir->op2) and ra_noreg(irs->r)) {
      ...
      while (phiset) {  // Check all left PHI operand registers.
         ...
         if (r != left) {  // Mismatch?
            if (!rset_test(as->freeset, r)) {  // PHI register blocked?
               ...
               if (!ra_iskref(ref) and irt_ismarked(IR(ref)->t)) {
                  rset_set(blocked, r);
                  if (ra_hasreg(left))
...
static void asm_phi_copyspill(ASMState* as)
{
   ...
   for (ir = IR(as->orignins - 1); ir->o == IR_PHI; ir--)
      if (ra_hasspill(ir->s) and ra_hasspill(IR(ir->op1)->s))
   ...
   if ((need & 1)) {  // Copy integer spill slots.
      ...
      for (ir = IR(as->orignins - 1); ir->o == IR_PHI; ir--) {
         if (ra_hasspill(ir->s)) {
            IRIns* irl = IR(ir->op1);
            if (ra_hasspill(irl->s) and !irt_isfp(ir->t)) {
...
{
   RegSet allow = ((!LJ_SOFTFP and irt_isfp(ir->t)) ? RSET_FPR : RSET_GPR) &
      ~as->phiset;
   ...
   if (ra_hasspill(irl->s) or ra_hasspill(irr->s))
   ...
   if ((afree & (afree - 1))) {  // Two or more free registers?
      ...
      if (ra_hasreg(irl->r) or ra_hasreg(irr->r))
         lj_trace_err(as->J, LJ_TRERR_NYIPHI);
      ra_spill(as, ir);
...
{
   // Fuse HREF + EQ/NE.
   if ((ir - 1)->o == IR_HREF and ir->op1 == as->curins - 1) {
      as->curins--;
...
static void asm_head_side(ASMState* as)
{
   ...
   if (as->snapno and as->topslot > as->parent->topslot) {
   ...
   for (i = as->stopins; i > REF_BASE; i--) {
      IRIns* ir = IR(i);
      RegSP rs;
      lj_assertA((ir->o == IR_SLOAD and (ir->op2 & IRSLOAD_PARENT)) ||
         (LJ_SOFTFP and ir->o == IR_HIOP) or ir->o == IR_PVAL,
   ...
   if (pass2) {
      for (i = as->stopins; i > REF_BASE; i--) {
         ...
         if (irt_ismarked(ir->t)) {
            ...
            mask = ((!LJ_SOFTFP and irt_isfp(ir->t)) ? RSET_FPR : RSET_GPR) & allow;
            if (mask == RSET_EMPTY)
   ...
   for (;;) {
      ...
      if (!LJ_SOFTFP and (live & RSET_FPR)) {
...
static void asm_setup_regsp(ASMState* as)
{
   ...
   for (lastir = IR(nins); ir < lastir; ir++) {
...
#if LJ_TARGET_ARM
      case IR_SLOAD:
         if (!((ir->op2 & IRSLOAD_TYPECHECK) or (ir + 1)->o == IR_HIOP))
            break;
         // fallthrough
      case IR_ALOAD: case IR_HLOAD: case IR_ULOAD: case IR_VLOAD:
         if (!LJ_SOFTFP and irt_isnum(ir->t)) break;
         ir->prev = (uint16_t)REGSP_HINT((rload & 15));
         rload = lj_ror(rload, 4);
         continue;
      case IR_TMPREF:
         if ((ir->op2 & IRTMPREF_OUT2) and as->evenspill < 4)
...
#if LJ_TARGET_X64 and LJ_ABI_WIN
         if (ir->op2 == IRCALL_lj_vm_next and as->evenspill < SPS_FIRST + 4)
            as->evenspill = SPS_FIRST + 4;
#else
         if (SPS_FIRST < 4 and ir->op2 == IRCALL_lj_vm_next and as->evenspill < 4)
            as->evenspill = 4;
#endif
         // fallthrough
      ...
      case IR_HIOP:
         switch ((ir - 1)->o) {
#if LJ_SOFTFP and LJ_TARGET_ARM
      ...
      case IR_LDEXP:
         ...
         // fallthrough
      case IR_POW:
         if (!LJ_SOFTFP and irt_isnum(ir->t)) {
            if (inloop)
               as->modset |= RSET_SCRATCH;
#if LJ_TARGET_X86
      ...
      case IR_DIV: case IR_MOD:
         if ((LJ_64 and LJ_SOFTFP) or !irt_isnum(ir->t)) {
            ir->prev = REGSP_HINT(RID_RET);
            if (inloop)
      ...
      case IR_BROL: case IR_BROR:
         if (!irref_isk(ir->op2) and !ra_hashint(IR(ir->op2)->r)) {
      ...
      case IR_ALOAD: case IR_HLOAD: case IR_ULOAD: case IR_VLOAD:
         ...
         break;
      case IR_CONV:
         if (irt_isfp(ir->t) or (ir->op2 & IRCONV_SRCMASK) == IRT_NUM ||
            (ir->op2 & IRCONV_SRCMASK) == IRT_FLOAT)
            break;
         // fallthrough
      default:
         // Propagate hints across likely 'op reg, imm' or 'op reg'.
         if (irref_isk(ir->op2) and !irref_isk(ir->op1) &&
...
void lj_asm_trace(jit_State* J, GCtrace* T)
{
   ...
   // Remove nops/renames left over from ASM restart due to LJ_TRERR_MCODELM.
   {
      ...
      if (ir->o == IR_NOP or ir->o == IR_RENAME) {
         do { ir--; nins--; } while (ir->o == IR_NOP or ir->o == IR_RENAME);
         T->nins = nins;
      }
   }
   ...
   */
   for (;;) {
      ...
      if (as->realign and J->curfinal->nins >= T->nins)
