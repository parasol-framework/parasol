...
static char* bcwrite_bytecode(BCWriteCtx* ctx, char* p, GCproto* pt)
{
   ...
   // Uses bc_op/setbc_op macros for endian-independent access to 64-bit BCIns.
   if ((pt->flags & PROTO_ILOOP) or pt->trace) {
      jit_State* J = L2J(sbufL(&ctx->sb));
      MSize i;
      for (i = 0; i < nbc; i++, q++) {
         BCOp op = bc_op(*q);
         if (op == BC_IFORL or op == BC_IITERL or op == BC_ILOOP or
            op == BC_JFORI) {
            setbc_op(q, (uint8_t)(op - BC_IFORL + BC_FORL));
         }
         else if (op == BC_JFORL or op == BC_JITERL or op == BC_JLOOP) {
            BCREG rd = bc_d(*q);
            *q = traceref(J, rd)->startins;  // Copy full 64-bit instruction
         }
      }
   }
#endif
