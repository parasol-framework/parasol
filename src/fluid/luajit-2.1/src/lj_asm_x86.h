...
static int asm_isk32(ASMState* as, IRRef ref, int32_t* k)
{
   if (irref_isk(ref)) {
      IRIns* ir = IR(ref);
      if (ir->o == IR_KNULL or !irt_is64(ir->t)) {
         *k = ir->i;
         return 1;
      }
...
static IRRef asm_fuseabase(ASMState* as, IRRef ref)
{
   ...
   if (irb->o == IR_FLOAD) {
      ...
      if (ira->o == IR_TNEW and ira->op1 <= LJ_MAX_COLOSIZE and
         !neverfuse(as) and noconflict(as, irb->op1, IR_NEWREF, 1)) {
         as->mrm.ofs = (int32_t)sizeof(GCtab);  /* Ofs to colocated array. */
         return irb->op1;  /* Table obj. */
      }
   }
   else if (irb->o == IR_ADD and irref_isk(irb->op2)) {
...
static void asm_fusearef(ASMState* as, IRIns* ir, RegSet allow)
{
   ...
   if (irref_isk(ir->op2)) {
      ...
      // Note: Constant ADD fusion disabled on 64-bit (has bad effects with negative index)
   ...
   if (ra_noreg(ir->r)) {
      ...
      case IR_UREFC:
         if (irref_isk(ir->op1)) {
            ...
            if (checki32(ofs) and checki32(ofs + 4)) {
...
static void asm_fusefref(ASMState* as, IRIns* ir, RegSet allow)
{
   ...
   if (irref_isk(ir->op1)) {
      ...
      else if (op1->o == IR_KPTR or op1->o == IR_KKPTR) {
...
static void asm_fusexref(ASMState* as, IRRef ref, RegSet allow)
{
   IRIns* ir = IR(ref);
   as->mrm.idx = RID_NONE;
   if (ir->o == IR_KPTR or ir->o == IR_KKPTR) {
      ...
      if (canfuse(as, ir) and ir->o == IR_ADD and ra_noreg(ir->r)) {
         ...
         if (asm_isk32(as, ir->op2, &as->mrm.ofs)) {  // Recognize x+ofs.
            ref = ir->op1;
            ir = IR(ref);
            if (!(ir->o == IR_ADD and canfuse(as, ir) and ra_noreg(ir->r)))
               goto noadd;
         }
         as->mrm.scale = XM_SCALE1;
         idx = ir->op1;
         ref = ir->op2;
         irx = IR(idx);
         if (!(irx->o == IR_BSHL or irx->o == IR_ADD)) {  // Try other operand.
            idx = ir->op2;
            ref = ir->op1;
            irx = IR(idx);
         }
         if (canfuse(as, irx) and ra_noreg(irx->r)) {
            if (irx->o == IR_BSHL and irref_isk(irx->op2) and IR(irx->op2)->i <= 3) {
               // Recognize idx<<b with b = 0-3, corresponding to sz = (1),2,4,8.
               idx = irx->op1;
               as->mrm.scale = (uint8_t)(IR(irx->op2)->i << 6);
            }
            else if (irx->o == IR_ADD and irx->op1 == irx->op2) {
...
// Fuse load of 64 bit IR constant into memory operand - GC64 only.
static Reg asm_fuseloadk64(ASMState* as, IRIns* ir)
{
   const uint64_t* k = &ir_k64(ir)->u64;
   if (checki32(dispofs(as, k))) {
      as->mrm.ofs = (int32_t)dispofs(as, k);
      as->mrm.base = RID_DISPATCH;
   }
   else if (checki32(mcpofs(as, k)) and checki32(mcpofs(as, k + 1)) and
      checki32(mctopofs(as, k)) and checki32(mctopofs(as, k + 1))) {
...
static Reg asm_fuseload(ASMState* as, IRRef ref, RegSet allow)
{
   ...
   if (ir->o == IR_KNUM) {
   ...
   else if (ref == REF_BASE or ir->o == IR_KINT64) {
      ...
      if (ir->o == IR_SLOAD) {
         if (!(ir->op2 & (IRSLOAD_PARENT | IRSLOAD_CONVERT)) and
            noconflict(as, ref, IR_RETF, 0) and
            !(LJ_GC64 and irt_isaddr(ir->t))) {
            as->mrm.base = (uint8_t)ra_alloc1(as, REF_BASE, xallow);
            as->mrm.ofs = 8 * ((int32_t)ir->op1 - 1 - LJ_FR2) +
               (!LJ_FR2 and (ir->op2 & IRSLOAD_FRAME) ? 4 : 0);
            as->mrm.idx = RID_NONE;
            return RID_MRM;
         }
      }
         ...
         if ((irt_isint(ir->t) or irt_isu32(ir->t) or irt_isaddr(ir->t)) and
            noconflict(as, ref, IR_FSTORE, 0)) {
            asm_fusefref(as, ir, xallow);
            return RID_MRM;
         }
      }
      else if (ir->o == IR_ALOAD or ir->o == IR_HLOAD or ir->o == IR_ULOAD) {
         if (noconflict(as, ref, (IROp)(ir->o + IRDELTA_L2S), 0) and
            !(LJ_GC64 and irt_isaddr(ir->t))) {
         ...
         */
         if ((!irt_typerange(ir->t, IRT_I8, IRT_U16)) and
            noconflict(as, ref, IR_XSTORE, 0)) {
            asm_fusexref(as, ir->op1, xallow);
            return RID_MRM;
         }
      }
      else if (ir->o == IR_VLOAD and !(LJ_GC64 and irt_isaddr(ir->t))) {
         asm_fuseahuref(as, ir->op1, xallow);
         as->mrm.ofs += 8 * ir->op2;
         return RID_MRM;
      }
   }
   if (ir->o == IR_FLOAD and ir->op1 == REF_NIL) {
      asm_fusefref(as, ir, RSET_EMPTY);
      return RID_MRM;
   }
   if (!(as->freeset & allow) and !emit_canremat(ref) and
      (allow == RSET_EMPTY or ra_hasspill(ir->s) or iscrossref(as, ref)))
      goto fusespill;
...
{
   if (is64 and !irt_is64(IR(ref)->t)) return ra_alloc1(as, ref, allow);
...
static int asm_count_call_slots(ASMState* as, const CCallInfo* ci, IRRef* args)
{
   ...
   if (LJ_ABI_WIN) {
      ...
      for (i = 0; i < nargs; i++)
         if (args[i] and irt_isfp(IR(args[i])->t)) {
...
static void asm_gencall(ASMState* as, const CCallInfo* ci, IRRef* args)
{
   ...
   for (n = 0; n < nargs; n++) {  // Setup args.
      ...
      if (r) {  // Argument is in a register.
         if (r < RID_MAX_GPR and ref < ASMREF_TMP1) {
            if (!(ir->o == IR_KINT or ir->o == IR_KNULL))
            ...
            else ra_allocref(as, ref, RID2RSET(r));
         }
      }
      else if (irt_isfp(ir->t)) {  // FP argument is on stack.
         lj_assertA(!(irt_isfloat(ir->t) and irref_isk(ref)), "unexpected float constant");
   ...
   if (!LJ_ABI_WIN and patchnfpr) *patchnfpr = fpr - REGARG_FIRSTFPR;
}
...
static void asm_setupresult(ASMState* as, IRIns* ir, const CCallInfo* ci)
{
   ...
   if (ra_used(ir)) {
      if (irt_isfp(ir->t)) {
         ...
         else ra_destreg(as, ir, RID_FPRET);
      }
      else if (hiop) ra_destpair(as, ir);
      else {
...
// Get function pointer for external call (64-bit only)
...
static void asm_retf(ASMState* as, IRIns* ir)
{
   ...
   int32_t delta = 1 + 1 + bc_a(*((const BCIns*)pc - 1));  // FR2 is always 1 in 64-bit
...
static void asm_conv(ASMState* as, IRIns* ir)
{
   ...
   int st64 = (st == IRT_I64 or st == IRT_U64 or (st == IRT_P64));
   ...
   if (irt_isfp(ir->t)) {
      ...
      if (stfp) {  // FP to FP conversion.
         ...
         Reg left = ((st == IRT_U32 or st == IRT_U64)) ?
         ...
         emit_mrm(as, irt_isnum(ir->t) ? XO_CVTSI2SD : XO_CVTSI2SS,
            dest | (((st64 or st == IRT_U32)) ? REX_64 : 0), left);
      }
      ...
      if (irt_isguard(ir->t)) {
         ...
         lj_assertA(irt_isint(ir->t) and st == IRT_NUM, "bad type for checked CONV");
         ...
         if (irt_isu64(ir->t)) {
            ...
            if (st == IRT_NUM) emit_rma(as, XO_ADDSD, tmp, &as->J->k64[LJ_K64_M2P64_31]);
            else emit_rma(as, XO_ADDSS, tmp, &as->J->k32[LJ_K32_M2P64_31]);
            emit_sjcc(as, CC_NS, l_end);
            ...
            if (irt_isu32(ir->t)) emit_rr(as, XO_MOV, dest, dest);  /* Zero hiword. */
            emit_mrm(as, op, dest | (((irt_is64(ir->t) or irt_isu32(ir->t))) ? REX_64 : 0), asm_fuseload(as, lref, RSET_FPR));
         }
      }
   }
   else if (st >= IRT_I8 and st <= IRT_U16) {  // Extend to 32 bit integer.
      Reg left, dest = ra_dest(as, ir, RSET_GPR);
      RegSet allow = RSET_GPR;
      x86Op op;
      lj_assertA(irt_isint(ir->t) or irt_isu32(ir->t), "bad type for CONV EXT");
      if (st == IRT_I8) {
         op = XO_MOVSXb;
         allow = RSET_GPR8;
         dest |= FORCE_REX;
      ...
      else if (st == IRT_I16) op = XO_MOVSXw;
      else op = XO_MOVZXw;
      ...
      if (irt_is64(ir->t)) {
         ...
         Reg dest = ra_dest(as, ir, RSET_GPR);
         if (st64 and !(ir->op2 & IRCONV_NONE)) {
            Reg left = asm_fuseload(as, lref, RSET_GPR);
...
static void asm_strto(ASMState* as, IRIns* ir)
{
   ...
   IRRef args[2];
   RegSet drop = RSET_SCRATCH;
   if ((drop & RSET_FPR) != RSET_FPR and ra_hasreg(ir->r)) rset_set(drop, ir->r);  /* WIN64 doesn't spill all FPRs. */
   ra_evictset(as, drop);
   asm_guardcc(as, CC_E);
...
static void asm_tvptr(ASMState* as, Reg dest, IRRef ref, MSize mode)
{
   if ((mode & IRTMPREF_IN1)) {
      IRIns* ir = IR(ref);
      if (irt_isnum(ir->t)) {
         if (irref_isk(ref) and !(mode & IRTMPREF_OUT1)) {
...
static void asm_href(ASMState* as, IRIns* ir, IROp merge)
{
   ...
   l_end = emit_label(as);
   if (merge == IR_NE) asm_guardcc(as, CC_E);  /* XI_JMP is not found by lj_asm_patchexit. */
   else if (destused) emit_loada(as, dest, niltvg(J2G(as->J)));
   ...
   if (irt_isnum(kt)) {
      ...
      else emit_rmro(as, XO_CMP, tmp | REX_64, dest, offsetof(Node, key.u64));
   }
   else {
      lj_assertA(irt_ispri(kt) and !irt_isnil(kt), "bad HREF key type");
...
static void asm_hrefk(ASMState* as, IRIns* ir)
{
   ...
   if (!irt_ispri(irkey->t)) {
      ...
      lj_assertA(irt_isnum(irkey->t) or irt_isgcv(irkey->t), "bad HREFK key type");
      // Assumes -0.0 is already canonicalized to +0.0.
...
static void asm_strref(ASMState* as, IRIns* ir)
{
   ...
   if (as->mrm.base == RID_NONE) emit_loadi(as, dest, as->mrm.ofs);
   else if (as->mrm.base == dest and as->mrm.idx == RID_NONE) {
      emit_gri(as, XG_ARITHi(XOg_ADD), dest | REX_GC64, as->mrm.ofs);
   }
   else emit_mrm(as, XO_LEA, dest | REX_GC64, RID_MRM);
}
...
static void asm_fxload(ASMState* as, IRIns* ir)
{
   ...
   x86Op xo;
   if (ir->o == IR_FLOAD) asm_fusefref(as, ir, RSET_GPR);
   else asm_fusexref(as, ir->op1, RSET_GPR);
   ...
   default:
      if (irt_is64(ir->t)) dest |= REX_64;
      else lj_assertA(irt_isint(ir->t) or irt_isu32(ir->t) or irt_isaddr(ir->t), "unsplit 64 bit load");
      xo = XO_MOV;
      break;
   }
...
static void asm_fxstore(ASMState* as, IRIns* ir)
{
   ...
   */
   if (irt_isi16(ir->t) or irt_isu16(ir->t) or irt_isfp(ir->t) or
      !asm_isk32(as, ir->op2, &k)) {
      RegSet allow8 = irt_isfp(ir->t) ? RSET_FPR :
         (irt_isi8(ir->t) or irt_isu8(ir->t)) ? RSET_GPR8 : RSET_GPR;
      src = osrc = ra_alloc1(as, ir->op2, allow8);
      rset_clear(allow, src);
   }

   if (ir->o == IR_FSTORE) asm_fusefref(as, IR(ir->op1), allow);
   else asm_fusexref(as, ir->op1, allow);
   ...
   if (ra_hasreg(src)) {
      ...
      default:
         if (irt_is64(ir->t)) src |= REX_64;
         else lj_assertA(irt_isint(ir->t) or irt_isu32(ir->t) or irt_isaddr(ir->t), "unsplit 64 bit store");
         xo = XO_MOVto;
         break;
      }
      ...
      if (irt_isi8(ir->t) or irt_isu8(ir->t)) {
         emit_i8(as, k);
         emit_mrm(as, XO_MOVmib, 0, RID_MRM);
      }
      else {
         lj_assertA(irt_is64(ir->t) or irt_isint(ir->t) or irt_isu32(ir->t) or irt_isaddr(ir->t), "bad store type");
         emit_i32(as, k);
...
{
   Reg tmp = RID_NONE;
   lj_assertA(irt_isnum(ir->t) or irt_ispri(ir->t) or irt_isaddr(ir->t) or
      (LJ_DUALNUM and irt_isint(ir->t)),
      ...
      if (ra_used(ir)) {
         ...
         else emit_mrm(as, dest < RID_MAX_GPR ? XO_MOV : XO_MOVSD, dest, RID_MRM);
      }
      else {
         RegSet gpr = RSET_GPR;
         if (irt_isaddr(ir->t)) {
   ...
   if (irt_type(ir->t) >= IRT_NUM) {
      lj_assertA(irt_isinteger(ir->t) or irt_isnum(ir->t), "bad load type %d", irt_type(ir->t));
...
static void asm_ahustore(ASMState* as, IRIns* ir)
{
   ...
   if (irt_isnum(ir->t)) {
      ...
      if (ra_hasreg(src)) {
         ...
         lj_assertA(irt_isaddr(ir->t) or (LJ_DUALNUM and irt_isinteger(ir->t)), "bad store type");
         emit_i32(as, irr->i);
         emit_mrm(as, XO_MOVmi, 0, RID_MRM);
      }
      as->mrm.ofs += 4;
      lj_assertA(LJ_DUALNUM and irt_isinteger(ir->t), "bad store type");
...
{
   int32_t ofs = 8 * ((int32_t)ir->op1 - 1 - LJ_FR2) + (!LJ_FR2 and (ir->op2 & IRSLOAD_FRAME) ? 4 : 0);
   IRType1 t = ir->t;
   Reg base;
   lj_assertA(!(ir->op2 & IRSLOAD_PARENT), "bad parent SLOAD"); /* Handled by asm_head_side(). */
   lj_assertA(irt_isguard(t) or !(ir->op2 & IRSLOAD_TYPECHECK), "inconsistent SLOAD variant");
   lj_assertA(LJ_DUALNUM or !irt_isint(t) or (ir->op2 & (IRSLOAD_CONVERT | IRSLOAD_FRAME | IRSLOAD_KEYINDEX)), "bad SLOAD type");
   if ((ir->op2 & IRSLOAD_CONVERT) and irt_isguard(t) and irt_isint(t)) {
      ...
      Reg dest = ra_dest(as, ir, allow);
      base = ra_alloc1(as, REF_BASE, RSET_GPR);
      lj_assertA(irt_isnum(t) or irt_isint(t) or irt_isaddr(t), "bad SLOAD type %d", irt_type(t));
      ...
      if ((ir->op2 & IRSLOAD_CONVERT)) {
         ...
         else emit_rmro(as, irt_isnum(t) ? XO_MOVSD : XO_MOV, dest, base, ofs);
      }
   }
   else {
      if (!(ir->op2 & IRSLOAD_TYPECHECK)) return;  /* No type check: avoid base alloc. */
      base = ra_alloc1(as, REF_BASE, RSET_GPR);
   }
   ...
   if ((ir->op2 & IRSLOAD_TYPECHECK)) {
      ...
      if (irt_type(t) >= IRT_NUM) {
         lj_assertA(irt_isinteger(t) or irt_isnum(t), "bad SLOAD type %d", irt_type(t));
         emit_u32(as, LJ_TISNUM << 15);
...
static void asm_x87load(ASMState* as, IRRef ref)
{
   ...
   if (ir->o == IR_KNUM) {
      ...
      else if (tvispone(tv)) emit_x87op(as, XI_FLD1);
      else emit_rma(as, XO_FLDq, XOg_FLDq, tv);
   }
   else if (ir->o == IR_CONV and ir->op2 == IRCONV_NUM_INT and !ra_used(ir) and
      !irref_isk(ir->op1) and mayfuse(as, ir->op1)) {
      IRIns* iri = IR(ir->op1);
      emit_rmro(as, XO_FILDd, XOg_FILDd, RID_ESP, ra_spill(as, iri));
   }
   else emit_mrm(as, XO_FLDq, XOg_FLDq, asm_fuseload(as, ref, RSET_EMPTY));
}
...
{
   if (irt_isnum(ir->t)) asm_fparith(as, ir, XO_SUBSD);
...
{
   if (irt_isnum(ir->t)) asm_fparith(as, ir, XO_MULSD);
   else asm_intarith(as, ir, XOg_X_IMUL);
}
...
{
   if (irt_isnum(ir->t)) asm_fparith(as, ir, XO_XORPS);
   else asm_neg_not(as, ir, XOg_NEG);
}
...
static void asm_bitshift(ASMState* as, IRIns* ir, x86Shift xs, x86Op xv)
{
   ...
   if (irref_isk(rref)) {  // Constant shifts.
      ...
      if (!xv and shift and (as->flags & JIT_F_BMI2)) {
   ...
   else if ((as->flags & JIT_F_BMI2) and xv) {  // BMI2 variable shifts.
      Reg left, right;
      dest = ra_dest(as, ir, RSET_GPR);
      ...
      right = irr->r;
      if (ra_noreg(right)) right = ra_allocref(as, rref, RID2RSET(RID_ECX));
      else if (right != RID_ECX) ra_scratch(as, RID2RSET(RID_ECX));
      emit_rr(as, XO_SHIFTcl, REX_64IR(ir, xs), dest);
      ra_noweak(as, right);
      if (right != RID_ECX) emit_rr(as, XO_MOV, RID_ECX, right);
   }
   ra_left(as, dest, ir->op1);
   /*
   ...
   if (irt_isnum(ir->t)) {
      ...
      if (cc & VCC_P) {  // Extra CC_P branch required?
         if (!(cc & VCC_U)) {
            ...
            as->loopinv = 2;
            if (as->realign) emit_sjcc(as, CC_P, as->mcp);
            else emit_jcc(as, CC_P, as->mcp);
         }
      }
      ...
      int32_t imm = 0;
      lj_assertA(irt_is64(ir->t) or irt_isint(ir->t) or
         irt_isu32(ir->t) or irt_isaddr(ir->t) or irt_isu8(ir->t),
         "bad comparison data type %d", irt_type(ir->t));
      // Swap constants (only for ABC) and fusable loads to the right.
      if (irref_isk(lref) or (!irref_isk(rref) and opisfusableload(leftop))) {
      ...
      if (asm_isk32(as, rref, &imm)) {
         ...
         int usetest = (imm == 0 and (cc & 0xa) != 0x2);
         if (usetest and irl->o == IR_BAND and irl + 1 == ir and !ra_used(irl)) {
            ...
            if (!asm_isk32(as, irl->op2, &imm)) {
               ...
               if (opisfusableload((IROp)irll->o) and
                  (irt_isi8(irll->t) or irt_isu8(irll->t))) {
         ...
         test_nofuse:
            ...
            Reg left;
            if (opisfusableload((IROp)irl->o) and
               ((irt_isu8(irl->t) and checku8(imm)) or
                  ((irt_isi8(irl->t) or irt_isi16(irl->t)) and checki8(imm)) or
                  (irt_isu16(irl->t) and checku16(imm) and checki8((int16_t)imm)))) {
               ...
               */
               ...
               if (left == RID_MRM) {  // Fusion succeeded?
                  if (irt_isu8(irl->t) or irt_isu16(irl->t))
                     cc >>= 4;  /* Need unsigned compare. */
                  asm_guardcc(as, cc);
                  emit_i8(as, imm);
                  emit_mrm(as, (irt_isi8(origt) or irt_isu8(origt)) ?
            ...
            if (usetest and left != RID_MRM) {
               ...
               x86Op xo = XO_TEST;
               if (irt_isu8(ir->t)) {
                  lj_assertA(ir->o == IR_EQ or ir->o == IR_NE, "bad usage");
                  xo = XO_TESTb;
            ...
            else emit_gmrmi(as, XG_ARITHi(XOg_CMP), r64 + left, imm);
         }
      }
      else {
...
static void asm_stack_check(ASMState* as, BCREG topslot, IRIns* irp, RegSet allow, ExitNo exitno)
{
   ...
   else ra_modified(as, r);
   emit_gri(as, XG_ARITHi(XOg_CMP), r | REX_GC64, (int32_t)(8 * topslot));
   if (ra_hasreg(pbase) and pbase != r)
      emit_rr(as, XO_ARITH(XOg_SUB), r | REX_GC64, pbase);
   else emit_rmro(as, XO_ARITH(XOg_SUB), r | REX_64, RID_DISPATCH, (int32_t)dispofs(as, &J2G(as->J)->jit_base));
...
static void asm_stack_restore(ASMState* as, SnapShot* snap)
{
   ...
   for (n = 0; n < nent; n++) {
      ...
      if ((sn & SNAP_KEYINDEX)) {
         ...
         lj_assertA(irt_ispri(ir->t) or irt_isaddr(ir->t) or (LJ_DUALNUM and irt_isinteger(ir->t)),
...
static void asm_head_root_base(ASMState* as)
{
   ...
   if (ra_hasreg(r)) {
      ra_free(as, r);
      if (rset_test(as->modset, r) or irt_ismarked(ir->t))
         ir->r = RID_INIT;  /* No inheritance for modified BASE register. */
      if (r != RID_BASE) emit_rr(as, XO_MOV, r | REX_GC64, RID_BASE);
   }
}
...
static RegSet asm_head_side_base(ASMState* as, IRIns* irp, RegSet allow)
{
   ...
   if (ra_hasreg(r)) {
      ...
      else if (ra_hasreg(irp->r) and rset_test(as->freeset, irp->r)) {
...
static void asm_tail_prep(ASMState* as)
{
   ...
   else p -= 5;  /* Space for exit branch (near jmp). */

   if (as->loopref) as->invmcp = as->mcp = p;
   else {
...
static uint32_t asm_x86_inslen(const uint8_t* p)
{
   ...
   while (true) {
      ...
      case 6:  /* Group 3. */
         if (p[1] & 0x38) x = 2;
         else if ((prefixes & 2) and (x == 0x66)) x = 4;
         goto mrm;
      case 7: /* VEX c4/c5. */
         if (x == 0x70) {
            x = *++p & 0x1f;
      ...
      case 9: mrm:  /* ModR/M and possibly SIB. */
         ...
         if ((x & 7) == 4) {
            result++;
            if (x < 0x40 and (p[1] & 7) == 5) result += 4;
         }
         return result;
      }
   }
...
void lj_asm_patchexit(jit_State* J, GCtrace* T, ExitNo exitno, MCode* target)
{
   ...
   if (len > 5 and p[len - 5] == XI_JMP and p + len - 6 + *(int32_t*)(p + len - 4) == px)
      *(int32_t*)(p + len - 4) = jmprel(J, p + len, target);
   // Do not patch parent exit for a stack check. Skip beyond vmstate update.
   for (; p < pe; p += asm_x86_inslen(p)) {
      intptr_t ofs = LJ_GC64 ? (p[0] & 0xf0) == 0x40 : 1;
      if (*(uint32_t*)(p + 2 + ofs) == statei and p[ofs + LJ_GC64 - 1] == XI_MOVmi)
         break;
   }
   lj_assertJ(p < pe, "instruction length decoder failed");
   for (; p < pe; p += asm_x86_inslen(p)) {
      if ((*(uint16_t*)p & 0xf0ff) == 0x800f and p + *(int32_t*)(p + 2) == px and
         p != pgc) {
         *(int32_t*)(p + 2) = jmprel(J, p + 6, target);
      }
      else if (*p == XI_CALL and
