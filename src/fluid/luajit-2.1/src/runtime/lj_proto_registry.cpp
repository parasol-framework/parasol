...
static fprototype * alloc_prototype(std::initializer_list<FluidType> ResultTypes,
   std::initializer_list<FluidType> ParamTypes, FProtoFlags Flags)
{
   size_t struct_size = sizeof(fprototype) + ParamTypes.size() * sizeof(FluidType);
   ...
   for (; idx < PROTO_MAX_RETURN_TYPES; ++idx) {
      proto->result_types[idx] = FluidType::Unknown;
   }

   // Copy parameter types

   FluidType *params = proto->param_types();
   idx = 0;
...
ERR reg_iface_prototype(std::string_view Interface, std::string_view Method, std::initializer_list<FluidType> ResultTypes,
   std::initializer_list<FluidType> ParamTypes, FProtoFlags Flags)
{
