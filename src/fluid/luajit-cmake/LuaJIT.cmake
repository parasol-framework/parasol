...
set(LUAJIT_OUTPUT_NAME "" CACHE STRING "Override the default LuaJIT library output name")
...
set(LUAJIT_GENVERSION_PATH ${LUAJIT_DIR}/src/host/genversion.lua)
set(LUAJIT_ROLLING_HEADER ${LUAJIT_DIR}/src/luajit_rolling.h)
set(LUAJIT_HEADER_OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/luajit.h)

if (EXISTS ${LUAJIT_GENVERSION_PATH} AND EXISTS ${LUAJIT_ROLLING_HEADER})
  ...
  add_custom_command(OUTPUT ${LUAJIT_HEADER_OUTPUT}
    COMMAND ${HOST_WINE} ${MINILUA_PATH} ${LUAJIT_GENVERSION_PATH}
    ARGS ${LUAJIT_ROLLING_HEADER}
         ${CMAKE_CURRENT_BINARY_DIR}/luajit_relver.txt
         ${LUAJIT_HEADER_OUTPUT}
    DEPENDS ${LUAJIT_ROLLING_HEADER}
    DEPENDS ${CMAKE_CURRENT_BINARY_DIR}/luajit_relver.txt
  )
else()
  add_custom_command(OUTPUT ${LUAJIT_HEADER_OUTPUT}
    COMMAND ${CMAKE_COMMAND} -E copy_if_different ${LUAJIT_DIR}/src/luajit.h ${LUAJIT_HEADER_OUTPUT}
    DEPENDS ${LUAJIT_DIR}/src/luajit.h
  )
endif()

add_custom_target(luajit_header ALL
  DEPENDS ${LUAJIT_HEADER_OUTPUT}
)
...
add_custom_command(OUTPUT ${BUILDVM_ARCH_H}
  ...
  DEPENDS minilua ${DASM_PATH} ${LUAJIT_HEADER_OUTPUT})
...
if(LUAJIT_OUTPUT_NAME)
  set_target_properties(libluajit PROPERTIES OUTPUT_NAME ${LUAJIT_OUTPUT_NAME})
elseif(MSVC)
