...
typedef union small_pool_struct {
  struct {
    ...
    LONG bytes_used;		/* how many bytes already used within pool */
    LONG bytes_left;		/* bytes still available in this pool */
  } hdr;
...
static const LONG first_pool_slop[JPOOL_NUMPOOLS] =
{
   1600,			/* first PERMANENT pool */
   16000			/* first IMAGE pool */
};

static const LONG extra_pool_slop[JPOOL_NUMPOOLS] =
{
...
METHODDEF(void *) alloc_small (j_common_ptr cinfo, int pool_id, LONG sizeofobject)
{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  small_pool_ptr hdr_ptr, prev_hdr_ptr;
  char * data_ptr;
  LONG odd_bytes, min_request, slop;

  /* Check for unsatisfiable request (do now to ensure no overflow below) */
  if (sizeofobject > (LONG) (MAX_ALLOC_CHUNK-SIZEOF(small_pool_hdr)))
  ...
  if (pool_id < 0 || pool_id >= JPOOL_NUMPOOLS) ERREXIT1(cinfo, JERR_BAD_POOL_ID, pool_id);	/* safety check */
  prev_hdr_ptr = NULL;
  hdr_ptr = mem->small_list[pool_id];
  while (hdr_ptr != NULL) {
    if (hdr_ptr->hdr.bytes_left >= sizeofobject) break;			/* found pool with enough space */
  ...
  if (hdr_ptr == NULL) {
    ...
    if (slop > (LONG) (MAX_ALLOC_CHUNK-min_request))
      slop = (LONG) (MAX_ALLOC_CHUNK-min_request);
...
METHODDEF(void FAR *) alloc_large (j_common_ptr cinfo, int pool_id, LONG sizeofobject)
{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  large_pool_ptr hdr_ptr;
  LONG odd_bytes;

  if (sizeofobject > (LONG) (MAX_ALLOC_CHUNK-SIZEOF(large_pool_hdr))) out_of_memory(cinfo, 3);	/* request exceeds malloc's ability */
  odd_bytes = sizeofobject % SIZEOF(ALIGN_TYPE);
  if (odd_bytes > 0) sizeofobject += SIZEOF(ALIGN_TYPE) - odd_bytes;
  if (pool_id < 0 || pool_id >= JPOOL_NUMPOOLS) ERREXIT1(cinfo, JERR_BAD_POOL_ID, pool_id);	/* safety check */
  hdr_ptr = (large_pool_ptr) jpeg_get_large(cinfo, sizeofobject + SIZEOF(large_pool_hdr));
  if (hdr_ptr == NULL) out_of_memory(cinfo, 4);	/* jpeg_get_large failed */
...
METHODDEF(JSAMPARRAY) alloc_sarray (j_common_ptr cinfo, int pool_id, JDIMENSION samplesperrow, JDIMENSION numrows)
{
   ...
   if (ltemp < (long) numrows) rowsperchunk = (JDIMENSION) ltemp;
   else rowsperchunk = numrows;
   mem->last_rowsperchunk = rowsperchunk;
   result = (JSAMPARRAY) alloc_small(cinfo, pool_id, (LONG) (numrows * SIZEOF(JSAMPROW)));

   currow = 0;
   while (currow < numrows) {
      rowsperchunk = MIN(rowsperchunk, numrows - currow);
      workspace = (JSAMPROW) alloc_large(cinfo, pool_id, (LONG) ((LONG) rowsperchunk * (LONG) samplesperrow * SIZEOF(JSAMPLE)));
...
METHODDEF(JBLOCKARRAY) alloc_barray (j_common_ptr cinfo, int pool_id, JDIMENSION blocksperrow, JDIMENSION numrows)
{
   ...
   mem->last_rowsperchunk = rowsperchunk;
   result = (JBLOCKARRAY) alloc_small(cinfo, pool_id, (LONG)(numrows * SIZEOF(JBLOCKROW)));
   currow = 0;
   while (currow < numrows) {
      rowsperchunk = MIN(rowsperchunk, numrows - currow);
      workspace = (JBLOCKROW) alloc_large(cinfo, pool_id, (LONG) ((LONG) rowsperchunk * (LONG) blocksperrow * SIZEOF(JBLOCK)));
      for (i = rowsperchunk; i > 0; i--) {
         result[currow++] = workspace;
...
/* Allocate the in-memory buffers for any unrealized virtual arrays */
{
  ...
  if (avail_mem >= maximum_space) max_minheights = 1000000000L;
  else {
...
/* Do backing store read or write of a virtual sample array */
{
   ...
   for (i = 0; i < (long) ptr->rows_in_mem; i += ptr->rowsperchunk) {
      ...
      if (rows <= 0) break; /* this chunk might be past end of file! */
      byte_count = rows * bytesperrow;

      if (writing) (*ptr->b_s_info.write_backing_store) (cinfo, & ptr->b_s_info, (void FAR *) ptr->mem_buffer[i], file_offset, byte_count);
      else (*ptr->b_s_info.read_backing_store) (cinfo, & ptr->b_s_info, (void FAR *) ptr->mem_buffer[i], file_offset, byte_count);
...
void jinit_memory_mgr (j_common_ptr cinfo)
{
  ...
  cinfo->mem = NULL;
  if ((SIZEOF(ALIGN_TYPE) & (SIZEOF(ALIGN_TYPE)-1)) != 0) ERREXIT(cinfo, JERR_BAD_ALIGN_TYPE);

  /* MAX_ALLOC_CHUNK must be representable as type LONG, and must be
   * a multiple of SIZEOF(ALIGN_TYPE).
   * Again, an "unreachable code" warning may be ignored here.
   * But a "constant too large" warning means you need to fix MAX_ALLOC_CHUNK.
   */

  test_mac = (LONG) MAX_ALLOC_CHUNK;
  if ((long) test_mac != MAX_ALLOC_CHUNK || (MAX_ALLOC_CHUNK % SIZEOF(ALIGN_TYPE)) != 0) ERREXIT(cinfo, JERR_BAD_ALLOC_CHUNK);

  max_to_use = jpeg_mem_init(cinfo);
  mem = (my_mem_ptr) jpeg_get_small(cinfo, SIZEOF(my_memory_mgr));
  if (mem == NULL) ERREXIT1(cinfo, JERR_OUT_OF_MEMORY, 0);
