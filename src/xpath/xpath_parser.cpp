...
// Dispatches to the appropriate computed constructor parser based on the leading
// keyword so each form can apply its specialised grammar.
...
// Parses computed element constructors, optionally accepting enclosed expressions for
// the element name and always parsing the content expression sequence.
...
// Parses computed attribute constructors which may provide the attribute name either as
// a literal QName or as an enclosed expression, followed by the attribute value
// expression.
...
// Parses computed text constructors by wrapping the enclosed expression inside a
// CONSTRUCTOR_CONTENT node so the evaluator can produce the resulting text node.
...
// Parses computed comment constructors mirroring the text constructor structure but
// targeting comment nodes in the AST.
...
// Parses computed processing-instruction constructors, supporting literal or computed
// targets along with the required content expression.
...
// Parses computed document constructors that evaluate their enclosed expression to
// populate a synthetic document node.
