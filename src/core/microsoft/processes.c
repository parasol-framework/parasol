...
void task_register_stdout(APTR Task, HANDLE Handle);
...
LONG winLaunchProcess(APTR Task, LPSTR commandline, LPSTR InitialDir, BYTE Group, BYTE InternalRedirect,
   struct winprocess **ProcessResult, char HideWindow, char *RedirectStdOut, char *RedirectStdErr, LONG *ProcessID)
{
   ...
   STARTUPINFO start;
   for (i=0; i < sizeof(STARTUPINFO); i++) ((char *)&start)[i] = 0;
   ...
   if ((process = calloc(1, sizeof(struct winprocess)))) {
      if (InternalRedirect) {
