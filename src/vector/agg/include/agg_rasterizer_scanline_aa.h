...
namespace agg
{
   ...
   template<class Clip=rasterizer_sl_clip_dbl> class rasterizer_scanline_aa {
   ...
   template<class Clip>
   void rasterizer_scanline_aa<Clip>::move_to(double x, double y) {
      if (m_outline.sorted()) reset();
   ...
   template<class Clip>
   void rasterizer_scanline_aa<Clip>::line_to(double x, double y) {
   ...
   void rasterizer_scanline_aa<Clip>::add_vertex(double x, double y, unsigned cmd) {
      if (is_move_to(cmd)) move_to(x, y);
      else if (is_vertex(cmd)) line_to(x, y);
      else if (is_close(cmd)) close_polygon();
   }
   ...
   AGG_INLINE bool rasterizer_scanline_aa<Clip>::rewind_scanlines() {
      if (m_auto_close) close_polygon();
      m_outline.sort_cells();
      if (m_outline.total_cells() == 0) return false;
