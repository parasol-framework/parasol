...
public:
    ...
    inline const rect_i& clip_box() const { return m_clip_box; }
    inline int xmin() const { return m_clip_box.x1; }
    inline int ymin() const { return m_clip_box.y1; }
    inline int xmax() const { return m_clip_box.x2; }
    inline int ymax() const { return m_clip_box.y2; }

    inline const rect_i& bounding_clip_box() const { return m_clip_box; }
    inline int bounding_xmin() const { return m_clip_box.x1; }
    inline int bounding_ymin() const { return m_clip_box.y1; }
    inline int bounding_xmax() const { return m_clip_box.x2; }
    inline int bounding_ymax() const { return m_clip_box.y2; }
    ...
    inline void copy_pixel(int x, int y, const color_type& c) {
        if (inbox(x, y)) m_ren->copy_pixel(x, y, c);
    }

    inline void blend_pixel(int x, int y, const color_type& c, cover_type cover) {
        if (inbox(x, y)) m_ren->blend_pixel(x, y, c, cover);
    }
        ...
        if (rc.x2 > 0) {
            ...
            while(rc.y2 > 0) {
                ...
                if (rw.ptr) {
                    ...
                    if (len > 0) {
                        if (x1src + len-1 > rw.x2) len -= x1src + len - rw.x2 - 1;
