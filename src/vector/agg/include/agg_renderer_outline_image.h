...
namespace agg
{
    ...
    public:
        ...
        color_type pixel(int x, int y) const {
            ...
            int y1 = ufloor(src_y);
            int y2 = y1 + 1;
            color_type pix1 = (y1 < 0) ? color_type::no_color() : m_source.pixel(m_source, x, y1);
            color_type pix2 = (y2 > h) ? color_type::no_color() : m_source.pixel(m_source, x, y2);
    ...
    public:
        ...
        template<class Source> void create(const Source& src)
        {
            ...
            for(y = 0; y < m_height; y++)
            {
                ...
                for(x = 0; x < m_width; x++) *d1++ = src.pixel(src, x, y);
            }
