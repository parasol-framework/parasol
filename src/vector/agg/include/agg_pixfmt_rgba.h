...
namespace agg
{
    ...
    template<class ColorT, class Order> struct blender_rgba
    {
        ...
        static AGG_INLINE void blend_pix(value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned alpha, unsigned cover=0)
        {
    ...
    template<class ColorT, class Order> struct blender_rgba_pre
    {
        ...
        static AGG_INLINE void blend_pix(value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned alpha, unsigned cover)
        {
   ...
   template<class ColorT, class Order> struct comp_op_rgba_clear {
        ...
        static AGG_INLINE void blend_pix(value_type* p, unsigned, unsigned, unsigned, unsigned, unsigned cover) {
            if (cover < 255) {
                cover = 255 - cover;
    ...
    template<class ColorT, class Order> struct comp_op_rgba_dst {
        ...
        static AGG_INLINE void blend_pix(value_type*, unsigned, unsigned, unsigned, unsigned, unsigned)
        {
        }
    };
    ...
    template<class ColorT, class Order> struct comp_op_rgba_dst_in
    {
        ...
        // Dca' = Dca.Sa
        // Da'  = Sa.Da
        static AGG_INLINE void blend_pix(value_type* p, unsigned, unsigned, unsigned, unsigned sa, unsigned cover)
        {
            if (cover < 255) {
    ...
    template<class ColorT, class Order> struct comp_op_rgba_dst_out {
        ...
        static AGG_INLINE void blend_pix(value_type* p, unsigned, unsigned, unsigned, unsigned sa, unsigned cover)
        {
            if (cover < 255) sa = (sa * cover + 255) >> 8;
            sa = base_mask - sa;
    ...
    template<class ColorT, class Order> struct comp_op_table_rgba {
        typedef typename ColorT::value_type value_type;
        typedef void (*comp_op_func_type)(value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned ca, unsigned cover);
    ...
    template<class BlenderPre> struct comp_adaptor_rgba {
        ...
        {
            BlenderPre::blend_pix(p, (cr * ca + base_mask) >> base_shift, (cg * ca + base_mask) >> base_shift,
               (cb * ca + base_mask) >> base_shift, ca, cover);
        }
    };
   ...
   template<class BlenderPre> struct comp_adaptor_clip_to_dst_rgba {
      ...
      static AGG_INLINE void blend_pix(unsigned op, value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned ca, unsigned cover)
      {
         ...
         BlenderPre::blend_pix(p, (cr * da + base_mask) >> base_shift, (cg * da + base_mask) >> base_shift,
            (cb * da + base_mask) >> base_shift, (ca * da + base_mask) >> base_shift, cover);
      }
   };
   ...
   template<class BlenderPre> struct comp_adaptor_clip_to_dst_rgba_pre {
        ...
        static AGG_INLINE void copy_or_blend_pix(value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned alpha, unsigned cover)
        {
            if (cover == 255) copy_or_blend_pix(p, cr, cg, cb, alpha);
                ...
                if (alpha) {
                    ...
                    else Blender::blend_pix(p, cr, cg, cb, alpha, cover);
                }
            }
        }
    };
    ...
    public:
        ...
        AGG_INLINE color_type pixel(int x, int y) const {
            auto p = (const value_type*)m_rbuf->row_ptr(y);
            if (p) {
                p += x << 2;
                return color_type(p[order_type::R], p[order_type::G], p[order_type::B], p[order_type::A]);
            }
        ...
        {
            auto p = (value_type*)m_rbuf->row_ptr(x, y, 1) + (x << 2);
            p[order_type::R] = c.r;
            p[order_type::G] = c.g;
            p[order_type::B] = c.b;
            p[order_type::A] = c.a;
        ...
        {
            cob_type::copy_or_blend_pix((value_type*)m_rbuf->row_ptr(x, y, 1) + (x << 2),
        ...
        {
            auto p = (value_type*)m_rbuf->row_ptr(x, y, len) + (x << 2);
            pixel_type v;
        ...
        void blend_solid_hspan(int x, int y, unsigned len, const color_type& c, const int8u* covers) {
            if (c.a) {
                ...
                do {
                    ...
                    else blender_type::blend_pix(p, c.r, c.g, c.b, alpha, *covers);
                    p += 4;
                    ++covers;
                }
                while(--len);
        ...
        void blend_color_hspan(int x, int y, unsigned len, const color_type* colors, const int8u* covers, int8u cover) {
            ...
            if (covers) {
                do  {
                    cob_type::copy_or_blend_pix(p, colors->r, colors->g, colors->b, colors->a, *covers++);
                    p += 4;
                    ++colors;
                }
                while(--len);
                ...
                if (cover == 255) {
                    do {
                        cob_type::copy_or_blend_pix(p, colors->r, colors->g, colors->b, colors->a);
                        p += 4;
                        ++colors;
                    }
                    while(--len);
                    ...
                    do {
                        cob_type::copy_or_blend_pix(p, colors->r, colors->g, colors->b, colors->a, cover);
                        p += 4;
                        ++colors;
                    }
                    while(--len);
        ...
        template<class Function> void for_each_pixel(Function f) {
            ...
            for(y = 0; y < height(); ++y) {
                ...
                if (r.ptr) {
                    ...
                    value_type *p = (value_type*)m_rbuf->row_ptr(r.x1, y, len) + (r.x1 << 2);
                    do {
                        f(p);
                        p += 4;
                    }
        ...
        template<class RenBuf2> void copy_from(const RenBuf2& from, int xdst, int ydst, int xsrc, int ysrc,unsigned len)
        {
        ...
        void blend_from(const SrcPixelFormatRenderer& from, int xdst, int ydst, int xsrc, int ysrc, unsigned len, int8u cover)
        {
            ...
            auto psrc = (value_type*)from.row_ptr(ysrc);
            if (psrc) {
                psrc += xsrc << 2;
                auto pdst = (value_type*)m_rbuf->row_ptr(xdst, ydst, len) + (xdst << 2);
                int incp = 4;
                if (xdst > xsrc) {
                    psrc += (len-1) << 2;
                    pdst += (len-1) << 2;
                ...
                if (cover == 255) {
                    do {
                        cob_type::copy_or_blend_pix(pdst, psrc[src_order::R], psrc[src_order::G], psrc[src_order::B], psrc[src_order::A]);
                        psrc += incp;
                        pdst += incp;
                    }
                    while(--len);
                    ...
                    do {
                        cob_type::copy_or_blend_pix(pdst, psrc[src_order::R], psrc[src_order::G], psrc[src_order::B], psrc[src_order::A], cover);
                        psrc += incp;
                        pdst += incp;
                    }
                    while(--len);
        ...
        template<class SrcPixelFormatRenderer>
        void blend_from_color(const SrcPixelFormatRenderer& from, const color_type& color, int xdst, int ydst,
                              int xsrc, int ysrc, unsigned len, int8u cover)
        {
            typedef typename SrcPixelFormatRenderer::value_type src_value_type;
            auto psrc = (src_value_type*)from.row_ptr(ysrc);
            if (psrc) {
                auto pdst = (value_type*)m_rbuf->row_ptr(xdst, ydst, len) + (xdst << 2);
                do {
                    cob_type::copy_or_blend_pix(pdst, color.r, color.g, color.b, color.a, (*psrc * cover + base_mask) >> base_shift);
                    ++psrc;
                    pdst += 4;
                }
                while(--len);
        ...
        template<class SrcPixelFormatRenderer>
        void blend_from_lut(const SrcPixelFormatRenderer& from, const color_type* color_lut, int xdst, int ydst,
                            int xsrc, int ysrc, unsigned len, int8u cover)
        {
            typedef typename SrcPixelFormatRenderer::value_type src_value_type;
            auto psrc = (src_value_type*)from.row_ptr(ysrc);
    ...
    public:
        ...
        color_type pixel(int x, int y) const {
            auto p = (value_type*)m_rbuf->row_ptr(y) + (x << 2);
            return color_type(p[order_type::R], p[order_type::G], p[order_type::B], p[order_type::A]);
        }

        void copy_pixel(int x, int y, const color_type& c) {
            blender_type::blend_pix(m_comp_op, (value_type*)m_rbuf->row_ptr(x, y, 1) + (x << 2),
                c.r, c.g, c.b, c.a, 255);
        }
        ...
        {
            do{
                blender_type::blend_pix(m_comp_op, (value_type*)m_rbuf->row_ptr(x, y++, 1) + (x << 2), c.r, c.g, c.b, c.a, *covers++);
            }
            while(--len);
        }
        ...
        void blend_color_hspan(int x, int y, unsigned len, const color_type* colors, const int8u* covers, int8u cover)
        {
            auto p = (value_type*)m_rbuf->row_ptr(x, y, len) + (x << 2);
            do {
                blender_type::blend_pix(m_comp_op, p, colors->r, colors->g, colors->b, colors->a, covers ? *covers++ : cover);
                p += 4;
                ++colors;
            }
            while(--len);
        ...
        void blend_color_vspan(int x, int y, unsigned len, const color_type* colors, const int8u* covers, int8u cover)
        {
            do {
                blender_type::blend_pix(m_comp_op, (value_type*)m_rbuf->row_ptr(x, y++, 1) + (x << 2),
                    colors->r, colors->g, colors->b, colors->a, covers ? *covers++ : cover);
                ++colors;
            }
            while(--len);
        ...
        template<class Function> void for_each_pixel(Function f)
        {
            ...
            for(y = 0; y < height(); ++y) {
                ...
                if (r.ptr) {
                    unsigned len = r.x2 - r.x1 + 1;
                    auto p = (value_type*)m_rbuf->row_ptr(r.x1, y, len) + (r.x1 << 2);
                    do {
                        f(p);
                        p += 4;
                    }
        ...
        {
            typedef typename SrcPixelFormatRenderer::order_type src_order;
            auto psrc = (const value_type*)from.row_ptr(ysrc);
            if (psrc) {
                psrc += xsrc << 2;
                auto pdst = (value_type*)m_rbuf->row_ptr(xdst, ydst, len) + (xdst << 2);
                ...
                do {
                    blender_type::blend_pix(m_comp_op, pdst, psrc[src_order::R], psrc[src_order::G], psrc[src_order::B], psrc[src_order::A], cover);
                    psrc += incp;
                    pdst += incp;
                }
                while(--len);
        ...
        {
            typedef typename SrcPixelFormatRenderer::value_type src_value_type;
            auto psrc = (const src_value_type*)from.row_ptr(ysrc);
            if (psrc) {
                auto pdst = (value_type*)m_rbuf->row_ptr(xdst, ydst, len) + (xdst << 2);
                do {
                    blender_type::blend_pix(m_comp_op, pdst, color.r, color.g, color.b, color.a, (*psrc * cover + base_mask) >> base_shift);
                    ++psrc;
                    pdst += 4;
                }
                while(--len);
        ...
        void blend_from_lut(const SrcPixelFormatRenderer& from, const color_type* color_lut,
                            int xdst, int ydst, int xsrc, int ysrc, unsigned len, int8u cover)
        {
            typedef typename SrcPixelFormatRenderer::value_type src_value_type;
            auto psrc = (const src_value_type*)from.row_ptr(ysrc);
            if (psrc) {
                ...
                do {
                    const color_type& color = color_lut[*psrc];
                    blender_type::blend_pix(m_comp_op, pdst, color.r, color.g, color.b, color.a, cover);
                    ++psrc;
                    pdst += 4;
                }
                while(--len);
    ...
    typedef pixfmt_alpha_blend_rgba<blender_rgba32, rendering_buffer, pixel32_type> pixfmt_rgba32;
    typedef pixfmt_alpha_blend_rgba<blender_argb32, rendering_buffer, pixel32_type> pixfmt_argb32;
    typedef pixfmt_alpha_blend_rgba<blender_abgr32, rendering_buffer, pixel32_type> pixfmt_abgr32;
    typedef pixfmt_alpha_blend_rgba<blender_bgra32, rendering_buffer, pixel32_type> pixfmt_bgra32;

    typedef pixfmt_alpha_blend_rgba<blender_rgba32_pre, rendering_buffer, pixel32_type> pixfmt_rgba32_pre;
    typedef pixfmt_alpha_blend_rgba<blender_argb32_pre, rendering_buffer, pixel32_type> pixfmt_argb32_pre;
    typedef pixfmt_alpha_blend_rgba<blender_abgr32_pre, rendering_buffer, pixel32_type> pixfmt_abgr32_pre;
    typedef pixfmt_alpha_blend_rgba<blender_bgra32_pre, rendering_buffer, pixel32_type> pixfmt_bgra32_pre;

    typedef pixfmt_alpha_blend_rgba<blender_rgba32_plain, rendering_buffer, pixel32_type> pixfmt_rgba32_plain;
    typedef pixfmt_alpha_blend_rgba<blender_argb32_plain, rendering_buffer, pixel32_type> pixfmt_argb32_plain;
    typedef pixfmt_alpha_blend_rgba<blender_abgr32_plain, rendering_buffer, pixel32_type> pixfmt_abgr32_plain;
    typedef pixfmt_alpha_blend_rgba<blender_bgra32_plain, rendering_buffer, pixel32_type> pixfmt_bgra32_plain;

    struct  pixel64_type { int16u c[4]; };
    typedef pixfmt_alpha_blend_rgba<blender_rgba64, rendering_buffer, pixel64_type> pixfmt_rgba64;
    typedef pixfmt_alpha_blend_rgba<blender_argb64, rendering_buffer, pixel64_type> pixfmt_argb64;
    typedef pixfmt_alpha_blend_rgba<blender_abgr64, rendering_buffer, pixel64_type> pixfmt_abgr64;
    typedef pixfmt_alpha_blend_rgba<blender_bgra64, rendering_buffer, pixel64_type> pixfmt_bgra64;

    typedef pixfmt_alpha_blend_rgba<blender_rgba64_pre, rendering_buffer, pixel64_type> pixfmt_rgba64_pre;
    typedef pixfmt_alpha_blend_rgba<blender_argb64_pre, rendering_buffer, pixel64_type> pixfmt_argb64_pre;
