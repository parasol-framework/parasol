...
namespace agg
{
    ...
    public:
        ...
        line_interpolator_aa2(renderer_type& ren, const line_parameters& lp, int ex, int ey) :
            line_interpolator_aa_base<Renderer>(ren, lp),
            m_di(lp.x1, lp.y1, lp.x2, lp.y2, ex, ey,
                 lp.x1 & ~line_subpixel_mask, lp.y1 & ~line_subpixel_mask, 0)
        {
            base_type::m_li.adjust_forward();
        ...
        bool step_hor() {
            ...
            *p1 = 0;
            ...
            base_type::m_ren.blend_solid_vspan(base_type::m_x, base_type::m_y - dy + 1,
               unsigned(p1 - p0), p0);
        ...
        bool step_ver()
        {
            ...
            *p1 = 0;
            ...
            base_type::m_ren.blend_solid_hspan(base_type::m_x - dx + 1,
               base_type::m_y, unsigned(p1 - p0), p0);
            return npix && ++base_type::m_step < base_type::m_count;
        }
    ...
    public:
        ...
        bool step_hor()
        {
            ...
            *p1 = 0;
            ...
            base_type::m_ren.blend_solid_vspan(base_type::m_x, base_type::m_y - dy + 1, unsigned(p1 - p0), p0);
            return npix && ++base_type::m_step < base_type::m_count;
        }
    ...
    public:
        ...
        static const int subpixel_shift = line_subpixel_shift;
        ...
        static const int aa_scale = 1 << aa_shift;
        static const int aa_mask  = aa_scale - 1;

        line_profile_aa() : m_subpixel_width(0), m_min_width(1.0), m_smoother_width(1.0)
        {
            for(int i = 0; i < aa_scale; i++) m_gamma[i] = (value_type)i;
        }

        template<class GammaF>
        line_profile_aa(double w, const GammaF& gamma_function) :
            m_subpixel_width(0), m_min_width(1.0), m_smoother_width(1.0)
        {
        ...
        template<class GammaF> void gamma(const GammaF& gamma_function) {
            for(int i = 0; i < aa_scale; i++) {
                m_gamma[i] = value_type(uround(gamma_function(double(i) / aa_mask) * aa_mask));
            }
        }
