...
namespace agg
{
    ...
    public:
        ...
        explicit renderer_mclip(pixfmt_type& pixf) : m_ren(pixf), m_curr_cb(0),
        ...
        color_type pixel(int x, int y) const {
            first_clip_box();
            do {
                if (m_ren.inbox(x, y)) return m_ren.ren().pixel(x, y);
            }
            while(next_clip_box());
        ...
        void blend_color_vspan(int x, int y, int len, const color_type* colors, const cover_type* covers,
           cover_type cover = cover_full)
        {
            first_clip_box();
            do {
