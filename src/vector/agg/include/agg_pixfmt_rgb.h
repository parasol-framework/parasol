...
template<class ColorT, class Order> struct blender_rgb_pre
{
   ...
   static inline void blend_pix(value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned alpha, unsigned cover)
   {
   ...
   static inline void blend_pix(value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned alpha)
   {
      alpha = color_type::base_mask - alpha;
...
public:
   ...
   inline void blend_pix(value_type* p, unsigned cr, unsigned cg, unsigned cb, unsigned alpha, unsigned cover = 0)
   {
...
private:
   inline void copy_or_blend_pix(value_type* p, const color_type& c, unsigned cover) {
      if (c.a) {
         ...
         else m_blender.blend_pix(p, c.r, c.g, c.b, alpha, cover);
      }
   }

   inline void copy_or_blend_pix(value_type* p, const color_type& c)
   {
      if (c.a) {
         if (c.a == base_mask) {
            p[order_type::R] = c.r;
         ...
         else m_blender.blend_pix(p, c.r, c.g, c.b, c.a);
      }
   }


public:
   explicit pixfmt_alpha_blend_rgb(rbuf_type& rb) : m_rbuf(&rb) { }
   ...
   inline       int8u* row_ptr(int y) { return m_rbuf->row_ptr(y); }
   inline const int8u* row_ptr(int y) const { return m_rbuf->row_ptr(y); }
   inline row_data     row(int y)     const { return m_rbuf->row(y); }

   inline int8u* pix_ptr(int x, int y) {
      return m_rbuf->row_ptr(y) + x * pix_width;
   }

   inline const int8u* pix_ptr(int x, int y) const {
      return m_rbuf->row_ptr(y) + x * pix_width;
   }

   inline static void make_pix(int8u* p, const color_type& c) {
   ...
   inline void blend_pixel(int x, int y, const color_type& c, int8u cover)
   {
      copy_or_blend_pix((value_type*)m_rbuf->row_ptr(x, y, 1) + x + x + x, c, cover);
   }

   inline void copy_hline(int x, int y, unsigned len, const color_type& c)
   {
   ...
   inline void copy_vline(int x, int y, unsigned len, const color_type& c)
   {
      do {
         value_type* p = (value_type*)m_rbuf->row_ptr(x, y++, 1) + x + x + x;
         p[order_type::R] = c.r;
   ...
   void blend_solid_hspan(int x, int y, unsigned len, const color_type& c, const int8u* covers) {
      if (c.a) {
         ...
         do {
            ...
            else m_blender.blend_pix(p, c.r, c.g, c.b, alpha, *covers);
   ...
   template<class Function> void for_each_pixel(Function f) {
      ...
      for (y = 0; y < height(); ++y) {
         ...
         if (r.ptr) {
            ...
            auto p = (value_type*)m_rbuf->row_ptr(r.x1, y, len) + r.x1 * 3;
            do {
               f(p);
               p += 3;
            } while (--len);
   ...
   void blend_from(const SrcPixelFormatRenderer& from, int xdst, int ydst, int xsrc, int ysrc, unsigned len, int8u cover)
   {
      ...
      if (psrc) {
         psrc += xsrc * 4;
         auto pdst = (value_type*)m_rbuf->row_ptr(xdst, ydst, len) + xdst * 3;
         ...
         if (cover == 255) {
            do {
               ...
               if (alpha) {
                  ...
                  else m_blender.blend_pix(pdst, psrc[src_order::R], psrc[src_order::G], psrc[src_order::B], alpha);
               }
               psrc += 4;
               pdst += 3;
            } while (--len);
