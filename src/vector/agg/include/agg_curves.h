...
namespace agg
{
    ...
    public:
        curve4_div() :
            m_approximation_scale(1.0), m_angle_tolerance(0.0), m_cusp_limit(0.0), m_count(0)
        {}
        ...
        curve4_div(const curve4_points& cp) : m_approximation_scale(1.0), m_angle_tolerance(0.0), m_count(0)
        {
            init(cp[0], cp[1], cp[2], cp[3], cp[4], cp[5], cp[6], cp[7]);
        }
    ...
    public:
        ...
        void init(double x1, double y1, double x2, double y2, double x3, double y3) {
           if (m_approximation_method == curve_inc) m_curve_inc.init(x1, y1, x2, y2, x3, y3);
           else m_curve_div.init(x1, y1, x2, y2, x3, y3);
        }
        ...
        void rewind(unsigned path_id) {
            if (m_approximation_method == curve_inc) m_curve_inc.rewind(path_id);
            else m_curve_div.rewind(path_id);
        }

        unsigned vertex(double* x, double* y) {
            if (m_approximation_method == curve_inc) return m_curve_inc.vertex(x, y);
    ...
    public:
        ...
        curve4(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4) : m_approximation_method(curve_div)
        {
            init(x1, y1, x2, y2, x3, y3, x4, y4);
        }

        curve4(const curve4_points& cp) : m_approximation_method(curve_div) {
            init(cp[0], cp[1], cp[2], cp[3], cp[4], cp[5], cp[6], cp[7]);
        }
        ...
        void init(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4)
        {
            if (m_approximation_method == curve_inc) m_curve_inc.init(x1, y1, x2, y2, x3, y3, x4, y4);
            else m_curve_div.init(x1, y1, x2, y2, x3, y3, x4, y4);
        }
