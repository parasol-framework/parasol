...
namespace agg
{
    ...
    line_profile_aa::value_type* line_profile_aa::profile(double w) {
        ...
        if(size > m_profile.size()) m_profile.resize(size);
        return &m_profile[0];
    }
    ...
    void line_profile_aa::set(double center_width, double smoother_width) {
        ...
        ch = ch_center;
        for (i=0; i < subpixel_center_width; i++) *ch++ = (value_type)val;

        for (i=0; i < subpixel_smoother_width; i++) {
           *ch_smoother++ = m_gamma[unsigned((base_val - base_val * (double(i) / subpixel_smoother_width)) * aa_mask)];
        }

        unsigned n_smoother = profile_size() - subpixel_smoother_width - subpixel_center_width - subpixel_scale*2;

        val = m_gamma[0];
        for (i = 0; i < n_smoother; i++) *ch_smoother++ = (value_type)val;

        ch = ch_center;
        for (i = 0; i < subpixel_scale*2; i++) *--ch = *ch_center++;
    }
}
