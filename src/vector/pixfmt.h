...
void alphablend(uint8_t *p, uint8_t Red, uint8_t Green, uint8_t Blue, uint8_t Alpha) noexcept
{
...
public:
   ...
   explicit pixfmt_psl(uint8_t *Data, int Width, int Height, int Stride, int BPP, ColourFormat &Format, bool Linear = false) : oR(0), oG(0), oB(0), oA(0) {
...
private:
   void pixel_order(uint8_t aoR, uint8_t aoG, uint8_t aoB, uint8_t aoA)
   {
      oR = aoR;
      oG = aoG;
      oB = aoB;
   ...
   static void blendHLine32(agg::pixfmt_psl *Self, int x, int y, unsigned len, const agg::rgba8 &c, int8u cover) noexcept
   {
      if (c.a) {
         ...
         if (alpha == 0xff) {
            uint32_t v;
            ((uint8_t *)&v)[Self->oR] = c.r;
            ((uint8_t *)&v)[Self->oG] = c.g;
            ((uint8_t *)&v)[Self->oB] = c.b;
            ((uint8_t *)&v)[Self->oA] = c.a;
            do {
               *(uint32_t *)p = v;
               p += sizeof(uint32_t);
            } while(--len);
   ...
   {
      if (c.a) {
         uint8_t *p = Self->mData + (y * Self->mStride) + (x / 3);
         uint32_t alpha = (uint32_t(c.a) * (cover + 1)) >> 8;
         if (alpha == 0xff) {
            uint32_t v;
            ((uint8_t *)&v)[Self->oR] = c.r;
   ...
   static void blendSolidHSpan24(agg::pixfmt_psl *Self, int x, int y, uint32_t len, const agg::rgba8 &c, const uint8_t *covers) noexcept
   {
      if (c.a) {
         uint8_t *p = Self->mData + (y * Self->mStride) + (x / 3);
         do {
   ...
   static void blendColorHSpan24(agg::pixfmt_psl *Self, int x, int y, uint32_t len, const agg::rgba8 *colors, const uint8_t *covers, uint8_t cover) noexcept
   {
      uint8_t *p = Self->mData + (y * Self->mStride) + (x / 3);
      if (covers) {
         do {
   ...
   static void copyColorHSpan24(agg::pixfmt_psl *Self, int x, int y, uint32_t len, const agg::rgba8 *colors) noexcept
   {
      uint8_t *p = Self->mData + (y * Self->mStride) + (x / 3);
      do {
          p[Self->oR] = colors->r;
   ...
   static void blend24RGB(uint8_t *p, uint8_t cr, uint8_t cg, uint8_t cb, uint8_t alpha) noexcept
   {
   ...
   static void blend24BGR(uint8_t *p, uint8_t cr, uint8_t cg, uint8_t cb, uint8_t alpha) noexcept
   {
   ...
   inline static void copy24BGR(uint8_t *p, uint8_t cr, uint8_t cg, uint8_t cb, uint8_t alpha) noexcept
   {
      if (alpha) {
         if (alpha == 0xff) {
            p[0] = cb;
   ...
   inline static void copy24RGB(uint8_t *p, uint8_t cr, uint8_t cg, uint8_t cb, uint8_t alpha) noexcept
   {
      if (alpha) {
         if (alpha == 0xff) {
            p[0] = cr;
   ...
   static void cover24RGB(uint8_t *p, uint8_t cr, uint8_t cg, uint8_t cb, uint8_t alpha, uint32_t cover) noexcept
   {
      if (cover == 255) copy24RGB(p, cr, cg, cb, alpha);
      else if (alpha) {
         alpha = (alpha * (cover + 1)) >> 8;
         if (alpha == 0xff) {
            p[0] = cr;
   ...
   static void cover24BGR(uint8_t *p, uint8_t cr, uint8_t cg, uint8_t cb, uint8_t alpha, uint32_t cover) noexcept
   {
      if (cover == 255) copy24BGR(p, cr, cg, cb, alpha);
      else if (alpha) {
         alpha = (alpha * (cover + 1)) >> 8;
         if (alpha == 0xff) {
            p[0] = cb;
