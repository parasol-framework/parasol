...
static const struct FieldArray AudioFields[];
static const struct FieldDef AudioFlags[];
   ...
   for (index=0; index < ARRAYSIZE(Self->Channels); index++) {
      if (Self->Channels[index].ChannelMID) {
         if (!AccessMemory(Self->Channels[index].ChannelMID, MEM_READ_WRITE, 2000, &Self->Channels[index].Channel)) {
            ...
            LogMsg("Failed to access channel %d, removing from channel list.", index);
...
structsize LoopSize: Must be set to sizeof(struct AudioLoop).
...
ERROR AUDIO_AddSample(objAudio *Self, struct sndAddSample *Args)
{
   ...
   LogMethod("Data: %p, Length: %d", Args->Data, Args->DataSize);
   ...
   if (CurrentTaskID() != Self->Head.TaskID) {
      LogErrorMsg("Illegal call - use WaitMsg() to add samples to Audio Servers.");
   ...
   if (handle >= Self->TotalSamples) {
      if (!ReallocMemory(Self->Samples, sizeof(struct AudioSample) * (Self->TotalSamples + 10), &Self->Samples, &Self->SamplesMID)) {
   ...
   struct AudioSample *sample = &Self->Samples[handle];
   ClearMemory(sample, sizeof(struct AudioSample));
...
{
   struct AudioSample *sample;
   OBJECTPTR stream;
   LONG shift, handle, bufferlength;

   if ((!Args) OR (!Args->SampleFormat)) return PostError(ERR_NullArgs);
   if ((!Args->Path) AND (!Args->ObjectID)) return PostError(ERR_NullArgs);

   if (Args->Path) LogMethod("Path: %s, Length: %d", Args->Path, Args->SampleLength);
   else LogMethod("Object: %d, Length: %d", Args->ObjectID, Args->SampleLength);
   ...
   if (CurrentTaskID() != Self->Head.TaskID) {
      LogErrorMsg("Illegal call - use WaitMsg() to add streams to Audio Servers.");
      return ERR_IllegalActionAttempt;
   }
   ...
   if (handle >= Self->TotalSamples) {
      LogMsg("Reallocating sample list.");
      if (!ReallocMemory(Self->Samples, sizeof(struct AudioSample) * (Self->TotalSamples + 10), &Self->Samples, &Self->SamplesMID)) {
         handle = Self->TotalSamples;
         Self->TotalSamples += 10;
      }
   ...
   if (bufferlength < 256) {
      LogMsg("Warning: Buffer length of %d is less than minimum byte size of 256.", bufferlength);
      bufferlength = 256;
   }

   #ifdef __linux__
      if (bufferlength < Self->AudioBufferSize) LogErrorMsg("Warning: Buffer length of %d is less than audio buffer size of %d.", bufferlength, Self->AudioBufferSize);
   #endif

   // Setup the audio sample

   sample = &Self->Samples[handle];
   ClearMemory(sample, sizeof(struct AudioSample));
   sample->Used         = TRUE;
   sample->SampleType   = Args->SampleFormat;
   ...
   if (!AccessObject(sample->StreamID, 5000, &stream)) {
      MSG("Filling the buffer with sample data from source object #%d.", sample->StreamID);

      acSeek(stream, sample->SeekStart, SEEK_START);
      acRead(stream, sample->Data, sample->BufferLength, NULL);
      ReleaseObject(stream);
   }
   else LogErrorMsg("Failed to access stream source #%d.", sample->StreamID);
...
{
   if ((!Args) OR (!Args->Handle) OR (!Args->Command)) return PostError(ERR_NullArgs);

   MSG("Command: %d, Handle: $%.8x, Data: %d", Args->Command, Args->Handle, Args->Data);

   WORD index = Args->Handle>>16;

   if (index >= ARRAYSIZE(Self->Channels)) {
      LogErrorMsg("Bad channel handle $%.8x.", Args->Handle);
      return ERR_Args;
   }
   ...
   LogErrorMsg("Unsupported command ID #%d.", Args->Command);
   return ERR_NoSupport;
}
...
static ERROR AUDIO_CloseChannels(objAudio *Self, struct sndCloseChannels *Args)
{
   ...
   LogBranch("Handle: $%.8x", Args->Handle);

   if ((index < 0) OR (index >= ARRAYSIZE(Self->Channels))) PostError(ERR_Args);
   ...
   if (Self->Channels[index].OpenCount <= 0) {
      ...
      LogMsg("Total number of channels reduced to %d.", Self->TotalChannels);
...
{
   objAudio *Self;
   if (!AccessObject((OBJECTID)Reference, 3000, &Self)) {
      LogF("Audio","Dead task reported by system - checking integrity of %d channels.", ARRAYSIZE(Self->Channels));

      WORD index;
      for (index=0; index < ARRAYSIZE(Self->Channels); index++) {
         if ((Self->Channels[index].TaskID IS Info->ProcessID) AND (Self->Channels[index].OpenCount > 0)) {
            LogMsg("Removed orphaned channel #%d.", index);
      ...
      LogF("Audio","Total number of channels reduced to %d.", Self->TotalChannels);
...
static void user_login(APTR Reference, APTR Info, LONG InfoSize)
{
   ...
   if (!AccessObject((OBJECTID)Reference, 3000, &Self)) {
      if (Self->Initialising IS FALSE) {
         LogF("~Audio","User login detected - reloading audio configuration.");
...
static ERROR AUDIO_Init(objAudio *Self, APTR Void)
{
   ...
   LogMsg("Subscribing to events.");

   FUNCTION call;

   SET_FUNCTION_STDC(call, task_removed);
   SubscribeEvent(EVID_SYSTEM_TASK_REMOVED, &call, (APTR)Self->Head.UniqueID, &Self->TaskRemovedHandle);

   SET_FUNCTION_STDC(call, user_login);
   SubscribeEvent(EVID_USER_STATUS_LOGIN, &call, (APTR)Self->Head.UniqueID, &Self->UserLoginHandle);
...
static ERROR AUDIO_NewObject(objAudio *Self, APTR Void)
{
   ...
   if (AllocMemory(Self->TotalSamples * sizeof(struct AudioSample), Self->Head.MemFlags, &Self->Samples, &Self->SamplesMID) != ERR_Okay) {
      return ERR_AllocMemory;
   }
...
#else
   if (!AllocMemory(sizeof(struct VolumeCtl) * 2, Self->Head.MemFlags|MEM_NO_CLEAR, &Self->VolumeCtl, &Self->VolumeCtlMID)) {
      StrCopy("Master", Self->VolumeCtl[0].Name, sizeof(Self->VolumeCtl[0].Name));
      Self->VolumeCtl[0].Flags = NULL;
...
static ERROR AUDIO_OpenChannels(objAudio *Self, struct sndOpenChannels *Args)
{
   ...
   LogMsg("Allocating %d channels from index %d.", Args->Total, index);

   if (Self->Flags & ADF_OVER_SAMPLING) total = Args->Total * 2;
   else total = Args->Total;

   if (!AllocMemory(sizeof(struct AudioChannel) * total, Self->Head.MemFlags|MEM_TASK, &Self->Channels[index].Channel, &Self->Channels[index].ChannelMID)) {
      ...
      if (Args->Commands > 0) {
         if (!AllocMemory(sizeof(struct AudioCommand) * Args->Commands, Self->Head.MemFlags|MEM_CALLER, &Self->Channels[index].Commands, &Self->Channels[index].CommandMID)) {
...
static ERROR AUDIO_RemoveSample(objAudio *Self, struct sndRemoveSample *Args)
{
   ...
   if ((!Args) OR (!Args->Handle)) return PostError(ERR_NullArgs);

   LogMethod("Sample: %d", Args->Handle);

   if ((Args->Handle < 0) OR (Args->Handle >= Self->TotalSamples)) return PostError(ERR_OutOfRange);
   ...
   if (CurrentTaskID() != Self->Head.TaskID) {
      LogErrorMsg("Illegal call - use WaitMsg() to remove samples from Audio Servers.");
      return ERR_IllegalActionAttempt;
   }
...
static ERROR AUDIO_SetVolume(objAudio *Self, struct sndSetVolume *Args)
{
   ...
   if (!Self->VolumeCtl) return PostError(ERR_NoSupport);
   ...
   LogBranch("%s: %.2f, Flags: $%.8x", Self->VolumeCtl[index].Name, Args->Volume, Args->Flags);
   ...
   if (Args->Flags & SVF_UNMUTE) {
      if ((snd_mixer_selem_has_capture_switch(elem)) AND (!snd_mixer_selem_has_playback_switch(elem))) {
         ...
         for (chn=0; chn <= SND_MIXER_SCHN_LAST; chn++) {
            snd_mixer_selem_set_playback_switch(elem, chn, 1);
         }
      }
      Self->VolumeCtl[index].Flags &= ~VCF_MUTE;
   }
      ...
      if ((snd_mixer_selem_has_capture_switch(elem)) AND (!snd_mixer_selem_has_playback_switch(elem))) {
         for (chn=0; chn <= SND_MIXER_SCHN_LAST; chn++) {
            snd_mixer_selem_set_capture_switch(elem, chn, 0);
         }
      }
      else if (snd_mixer_selem_has_playback_switch(elem)) {
         for (chn=0; chn <= SND_MIXER_SCHN_LAST; chn++) {
            snd_mixer_selem_set_playback_switch(elem, chn, 0);
         }
      }
      Self->VolumeCtl[index].Flags |= VCF_MUTE;
   }
   ...
   if (((Args->Volume < 0) OR (Args->Volume > 1000)) AND (Args->Volume != -1)) {
      LogErrorMsg("Invalid volume: %.2f", Args->Volume);
...
static ERROR GET_Device(objAudio *Self, STRING *Value)
{
   *Value = Self->prvDevice;
   return ERR_Okay;
}

static ERROR SET_Device(objAudio *Self, STRING Value)
{
...
static ERROR GET_VolumeCtl(objAudio *Self, struct VolumeCtl **Value)
{
   ...
   if (Self->VolumeCtl) {
      WORD i;
      for (i=0; Self->VolumeCtl[i].Name[0]; i++) {
         Self->VolumeCtl[i].Size = sizeof(struct VolumeCtl);
      }
...
static ERROR SetInternalVolume(objAudio *Self, struct AudioChannel *Channel)
{
   FLOAT leftvol, rightvol;

   if ((!Self) OR (!Channel)) return PostError(ERR_Args);
   ...
   for (index=0; index < ARRAYSIZE(Self->Channels); index++) {
      if ((Self->Channels[index].Channel) AND (Self->Channels[index].Commands)) {
         ...
         if (Self->Channels[index].MixLeft <= 0) {
            ...
            total = 0;
            oldpos = Self->Channels[index].Position;

            // Skip start and end signals

            command = Self->Channels[index].Commands;
            while ((command->CommandID IS CMD_START_SEQUENCE) OR (command->CommandID IS CMD_END_SEQUENCE)) {
               total++;
               command++;
            }
            ...
            while ((command->CommandID) AND (command->CommandID != CMD_END_SEQUENCE) AND (command->CommandID != CMD_START_SEQUENCE)) {
               switch(command->CommandID) {
                  case CMD_CONTINUE:       COMMAND_Continue(Self, command->Handle); break;
                  case CMD_FADE_IN:        COMMAND_FadeIn(Self, command->Handle); break;
                  case CMD_FADE_OUT:       COMMAND_FadeOut(Self, command->Handle); break;
                  case CMD_MUTE:           COMMAND_Mute(Self, command->Handle, command->Data); break;
                  case CMD_PLAY:           COMMAND_Play(Self, command->Handle, command->Data); break;
                  case CMD_SET_FREQUENCY:  COMMAND_SetFrequency(Self, command->Handle, command->Data); break;
                  case CMD_SET_LENGTH:     COMMAND_SetLength(Self, command->Handle, command->Data); break;
                  case CMD_SET_PAN:        COMMAND_SetPan(Self, command->Handle, command->Data); break;
                  case CMD_SET_RATE:       COMMAND_SetRate(Self, command->Handle, command->Data); break;
                  case CMD_SET_SAMPLE:     COMMAND_SetSample(Self, command->Handle, command->Data); break;
                  case CMD_SET_VOLUME:     COMMAND_SetVolume(Self, command->Handle, command->Data); break;
                  case CMD_STOP:           COMMAND_Stop(Self, command->Handle); break;
                  case CMD_STOP_LOOPING:   COMMAND_StopLooping(Self, command->Handle); break;
                  case CMD_SET_POSITION:   COMMAND_SetPosition(Self, command->Handle, command->Data); break;
...
static ERROR audio_timer(objAudio *Self, LARGE Elapsed, LARGE CurrentTime)
{
   ...
   if (spaceleft < 0) {
      LogErrorMsg("avail_update() %s", snd_strerror(spaceleft));

      errcount++;
      if (!(errcount % 50)) {
         LogErrorMsg("Broken audio - attempting fix...");

         acDeactivate(Self);

         if (acActivate(Self) != ERR_Okay) {
            if ((Self->Flags & ADF_SERVICE_MODE) AND (errcount < 1000)) {
               LogErrorMsg("Audio error unrecoverable - will keep trying.");
            }
            else {
               LogErrorMsg("Audio error is terminal, self-destructing...");
   ...
   while (spaceleft) {
      ...
      buffer += elements * Self->SampleBitSize;
      spaceleft -= elements;
   }
   ...
   if (Self->Handle) {
      if ((err = snd_pcm_writei(Self->Handle, Self->AudioBuffer, space)) < 0) {
         ...
         if (err IS -EPIPE) {
            ...
            LogMsg("A buffer underrun has occurred.");
            ...
            if (code IS SND_PCM_STATE_XRUN) {
               ...
               else LogErrorMsg("snd_pcm_prepare() %s", snd_strerror(err));
            }
            else if (code IS SND_PCM_STATE_DRAINING) {
               LogMsg("Status: Draining");
            }
         }
         else LogErrorMsg("snd_pcm_writei() %d %s", err, snd_strerror(err));
      }
   }
...
static void load_config(objAudio *Self)
{
   ...
   if (!CreateObject(ID_CONFIG, 0, &config,
      ...
      if (!GetFields(config, FID_Entries|TPTR,     &entries,
                             FID_AmtEntries|TLONG, &amtentries,
                             TAGEND)) {
         ...
         if (i < amtentries) {
            ...
            if (!AllocMemory(sizeof(struct VolumeCtl) * (total + 1), Self->Head.MemFlags|MEM_NO_CLEAR, &Self->VolumeCtl, &Self->VolumeCtlMID)) {
         ...
         LogMsg("Loaded %d mixer entries from user audio configuration.", total);
      }
...
static ERROR init_audio(objAudio *Self)
{
   ...
   long pmin, pmax;
   int dir;
   struct VolumeCtl *volctl;
   MEMORYID volmid;
   WORD voltotal;
   UBYTE name[32], pcm_name[32];

   if (Self->Handle) {
      LogMsg("Audio system is already active.");
      return ERR_Okay;
   }

   snd_ctl_card_info_alloca(&info);

   LogMsg("Initialising sound card device.");
   ...
   if (StrMatch("default", pcm_name) != ERR_Okay) {
      ...
      if ((snd_card_next(&card) < 0) OR (card < 0)) {
         LogErrorMsg("There are no sound cards supported by audio drivers.");
         return ERR_NoSupport;
      }
      ...
      while (card >= 0) {
         ...
         if ((err = snd_ctl_open(&ctlhandle, name, 0)) >= 0) {
            if ((err = snd_ctl_card_info(ctlhandle, info)) >= 0) {
               ...
               LogMsg("Identified card %s, name %s", cardid, cardname);
                  ...
                  if ((err = snd_mixer_attach(mixhandle, name)) >= 0) {
                     if ((err = snd_mixer_selem_register(mixhandle, NULL, NULL)) >= 0) {
                        if ((err = snd_mixer_load(mixhandle)) >= 0) {
                           ...
                           LogMsg("Card %s has %d mixer controls.", cardid, voltotal);
                        ...
                        else LogErrorMsg("snd_mixer_load() %s", snd_strerror(err));
                     }
                     else LogErrorMsg("snd_mixer_selem_register() %s", snd_strerror(err));
                  }
                  else LogErrorMsg("snd_mixer_attach() %s", snd_strerror(err));
                  snd_mixer_close(mixhandle);
               }
               else LogErrorMsg("snd_mixer_open() %s", snd_strerror(err));
            }
next_card:
            snd_ctl_close(ctlhandle);
         }
   ...
   if ((err = snd_mixer_open(&Self->MixHandle, 0)) < 0) {
      LogErrorMsg("snd_mixer_open() %s", snd_strerror(err));
      return ERR_Failed;
   }

   if ((err = snd_mixer_attach(Self->MixHandle, pcm_name)) < 0) {
      LogErrorMsg("snd_mixer_attach() %s", snd_strerror(err));
      return ERR_Failed;
   }

   if ((err = snd_mixer_selem_register(Self->MixHandle, NULL, NULL)) < 0) {
      LogErrorMsg("snd_mixer_selem_register() %s", snd_strerror(err));
      return ERR_Failed;
   }

   if ((err = snd_mixer_load(Self->MixHandle)) < 0) {
      LogErrorMsg("snd_mixer_load() %s", snd_strerror(err));
      return ERR_Failed;
   }
   ...
   LogMsg("%d mixer controls have been reported by alsa.", voltotal);

   if (voltotal < 1) {
      LogErrorMsg("Aborting due to lack of mixers for the sound device.");
      return ERR_NoSupport;
   }

   if (!AllocMemory(sizeof(struct VolumeCtl) * (voltotal + 1), Self->Head.MemFlags|MEM_NO_CLEAR, &volctl, &volmid)) {
      index = 0;
      for (elem=snd_mixer_first_elem(Self->MixHandle); elem; elem=snd_mixer_elem_next(elem)) {
         ...
         MSG("Mixer Control '%s',%i", snd_mixer_selem_id_get_name(sid), snd_mixer_selem_id_get_index(sid));
         ...
         if (!(flags & VCF_MONO)) {
            for (channel=0; channel < ARRAYSIZE(glAlsaConvert); channel++) {
               if (snd_mixer_selem_has_playback_channel(elem, (int)glAlsaConvert[channel]))   {
                  long vol;
                  snd_mixer_selem_get_playback_volume(elem, glAlsaConvert[channel], &vol);
         ...
         if ((snd_mixer_selem_has_capture_switch(elem)) AND (!snd_mixer_selem_has_playback_switch(elem))) {
            for (channel=0; channel < ARRAYSIZE(glAlsaConvert); channel++) {
               flags |= VCF_MUTE;
               snd_mixer_selem_set_capture_switch(elem, channel, 0);
            }
         }
         else if (snd_mixer_selem_has_playback_switch(elem)) {
            for (channel=0; channel < ARRAYSIZE(glAlsaConvert); channel++) {
   ...
   LogMsg("ALSA bit rate: %d", Self->BitDepth);
   ...
   LogMsg("Using period time of %d, buffer time %d", period_time, buffer_time);
   ...
   LogMsg("Using period frame size of %d, buffer size of %d", (LONG)periodsize, (LONG)buffersize);
   ...
   // Retreive ALSA buffer sizes

   err = snd_pcm_hw_params_get_periods(hwparams, &Self->Periods, &dir);
   ...
   LogMsg("Total Periods: %d, Period Size: %d, Buffer Size: %d (bytes)", Self->Periods, Self->PeriodSize, Self->AudioBufferSize);
   ...
   if (!AllocMemory(Self->AudioBufferSize, MEM_DATA, &Self->AudioBuffer, NULL)) {
      ...
      if ((oldctl) AND (Self->Flags & ADF_SYSTEM_WIDE)) {
         LogMsg("Applying preset volumes to alsa.");
      ...
      else LogMsg("Skipping preset volumes.");
...
static const struct FieldArray AudioFields[] = {
   ...
   { "OutputRate",    FDF_LONG|FDF_RI,   0, NULL, SET_OutputRate },
   { "InputRate",     FDF_LONG|FDF_RI,   0, NULL, NULL },
   { "Quality",       FDF_LONG|FDF_RW,   0, NULL, SET_Quality },
   { "Flags",         FDF_LONGFLAGS|FDF_RI, (MAXINT)&AudioFlags, NULL, NULL },
   { "TotalChannels", FDF_LONG|FDF_R,    0, NULL, NULL },
   { "BitDepth",      FDF_LONG|FDF_RI,   0, NULL, SET_BitDepth },
   { "Periods",       FDF_LONG|FDF_RI,   0, NULL, SET_Periods },
   { "PeriodSize",    FDF_LONG|FDF_RI,   0, NULL, SET_PeriodSize },
   // VIRTUAL FIELDS
   { "Device",        FDF_STRING|FDF_RW,  0, GET_Device,       SET_Device },
   { "MasterVolume",  FDF_DOUBLE|FDF_RW,  0, GET_MasterVolume, SET_MasterVolume },
   { "Mute",          FDF_LONG|FDF_RW,    0, GET_Mute,         SET_Mute },
   { "Stereo",        FDF_LONG|FDF_RW,    0, GET_Stereo,       SET_Stereo },
   { "VolumeCtl",     FDF_POINTER|FDF_R,  0, GET_VolumeCtl,    NULL },
   END_FIELD
};
