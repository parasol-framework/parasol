...
GLOBAL(void) jpeg_CreateDecompress (j_decompress_ptr cinfo, int version, LONG structsize)
{
  int i;
   ...
   if (version != JPEG_LIB_VERSION) ERREXIT2(cinfo, JERR_BAD_LIB_VERSION, JPEG_LIB_VERSION, version);
   if (structsize != SIZEOF(struct jpeg_decompress_struct)) ERREXIT2(cinfo, JERR_BAD_STRUCT_SIZE, (int)SIZEOF(struct jpeg_decompress_struct), (int)structsize);
   ...
   */
   ...
   for (i = 0; i < NUM_QUANT_TBLS; i++) cinfo->quant_tbl_ptrs[i] = NULL;
...
GLOBAL(int) jpeg_read_header (j_decompress_ptr cinfo, boolean require_image)
{
  ...
  if (cinfo->global_state != DSTATE_START && cinfo->global_state != DSTATE_INHEADER) ERREXIT1(cinfo, JERR_BAD_STATE, cinfo->global_state);
  ...
  switch (retcode) {
     ...
     case JPEG_REACHED_EOI:
        if (require_image) ERREXIT(cinfo, JERR_NO_IMAGE);
...
GLOBAL(boolean) jpeg_input_complete (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->global_state < DSTATE_START || cinfo->global_state > DSTATE_STOPPING) ERREXIT1(cinfo, JERR_BAD_STATE, cinfo->global_state);
...
GLOBAL(boolean) jpeg_finish_decompress (j_decompress_ptr cinfo)
{
  if ((cinfo->global_state == DSTATE_SCANNING || cinfo->global_state == DSTATE_RAW_OK) && ! cinfo->buffered_image) {
    ...
    if (cinfo->output_scanline < cinfo->output_height) ERREXIT(cinfo, JERR_TOO_LITTLE_DATA);
