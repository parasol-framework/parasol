// Auto-generated by idl-c.fluid

static const struct FieldDef clClipboardResponse[] = {
   { "Cancel", 0x00000000 },
   { "NoAll", 0x00000000 },
   { "Yes", 0x00000001 },
   { "YesAll", 0x00000002 },
   { "No", 0x00000003 },
   { NULL, 0 }
};

static const struct FieldDef clClipboardFlags[] = {
   { "Wait", 0x00000001 },
   { "Host", 0x00000002 },
   { NULL, 0 }
};

FDEF maAddFile[] = { { "Datatype", FD_LONG }, { "Path", FD_STR }, { "Flags", FD_LONG }, { 0, 0 } };
FDEF maAddObject[] = { { "Datatype", FD_LONG }, { "Object", FD_OBJECTID }, { "Flags", FD_LONG }, { 0, 0 } };
FDEF maAddObjects[] = { { "Datatype", FD_LONG }, { "Objects", FD_PTR }, { "Flags", FD_LONG }, { 0, 0 } };
FDEF maGetFiles[] = { { "Datatype", FD_LONG }, { "Index", FD_LONG }, { "Files", FD_ARRAY|FD_STR|FD_ALLOC|FD_RESULT }, { "Flags", FD_LONG|FD_RESULT }, { 0, 0 } };
FDEF maAddText[] = { { "String", FD_STR }, { 0, 0 } };
FDEF maPasteFiles[] = { { "Dest", FD_STR }, { "Target", FD_OBJECTID }, { 0, 0 } };
FDEF maDelete[] = { { "Datatype", FD_LONG }, { 0, 0 } };
FDEF maDeleteFiles[] = { { "Target", FD_OBJECTID }, { 0, 0 } };

static const struct MethodArray clClipboardMethods[] = {
   { -1, (APTR)CLIPBOARD_AddFile, "AddFile", maAddFile, sizeof(struct clipAddFile) },
   { -2, (APTR)CLIPBOARD_AddObject, "AddObject", maAddObject, sizeof(struct clipAddObject) },
   { -3, (APTR)CLIPBOARD_AddObjects, "AddObjects", maAddObjects, sizeof(struct clipAddObjects) },
   { -4, (APTR)CLIPBOARD_GetFiles, "GetFiles", maGetFiles, sizeof(struct clipGetFiles) },
   { -5, (APTR)CLIPBOARD_AddText, "AddText", maAddText, sizeof(struct clipAddText) },
   { -6, (APTR)CLIPBOARD_PasteFiles, "PasteFiles", maPasteFiles, sizeof(struct clipPasteFiles) },
   { -7, (APTR)CLIPBOARD_Delete, "Delete", maDelete, sizeof(struct clipDelete) },
   { -8, (APTR)CLIPBOARD_DeleteFiles, "DeleteFiles", maDeleteFiles, sizeof(struct clipDeleteFiles) },
   { 0, 0, 0, 0, 0 }
};

static const struct ActionArray clClipboardActions[] = {
   { AC_ActionNotify, (APTR)CLIPBOARD_ActionNotify },
   { AC_Clear, (APTR)CLIPBOARD_Clear },
   { AC_DataFeed, (APTR)CLIPBOARD_DataFeed },
   { AC_Free, (APTR)CLIPBOARD_Free },
   { AC_GetVar, (APTR)CLIPBOARD_GetVar },
   { AC_Init, (APTR)CLIPBOARD_Init },
   { AC_NewObject, (APTR)CLIPBOARD_NewObject },
   { 0, 0 }
};

