...
ERROR StrReplace(CSTRING Src, STRING Keyword, STRING Replacement, STRING * Result, LONG Flags);
...
ERROR StrFormatDate(STRING Buffer, LONG Size, STRING Format, struct DateTime * Time);
...
ERROR MoveFile(CSTRING Source, CSTRING Dest, FUNCTION *Feedback);
...
ERROR CopyFile(CSTRING Source, CSTRING Dest, FUNCTION *Feedback);
ERROR SetDocView(CSTRING Path, CSTRING Document);
CSTRING GetDocView(CSTRING Path);
ERROR DeleteFile(CSTRING Path, FUNCTION *);
...
ERROR ReadFile(CSTRING Path, APTR Buffer, LONG BufferSize, LONG * Result);
