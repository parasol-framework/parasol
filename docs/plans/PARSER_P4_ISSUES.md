...
- **Operator emission still relies on legacy helpers.** `bcemit_arith` and `bcemit_comp` continue to juggle registers directly (including manual `freereg` adjustments) rather than exposing allocator/CFG-driven contracts through the AST pipeline, keeping low-level bytecode layout intertwined with parsing concerns.【F:src/fluid/luajit-2.1/src/parser/parse_operators.cpp†L34-L163】
...
| --- | --- | --- |
| Operator emission | `bcemit_arith`, `bcemit_comp` | Legacy helpers still discharge operands and decrement `freereg` directly instead of operating via allocator/CFG APIs.【F:src/fluid/luajit-2.1/src/parser/parse_operators.cpp†L34-L163】 |
