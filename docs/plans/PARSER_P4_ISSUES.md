...
- **Assignment and value storage remain manual.** Plain and compound assignments call `bcemit_store`, `lex_state.assign_adjust`, and reset `freereg` explicitly instead of flowing through LValue descriptors and allocator-managed slots, so statement nodes still mirror legacy register choreography. Register reset in `emit_plain_assignment` now flows through `RegisterGuard`, but the broader allocator/LValue shift is still pending.【F:src/fluid/luajit-2.1/src/parser/ir_emitter.cpp†L1022-L1108】
...
| Legacy operator path | `bcemit_binop_left` (legacy) | Delegates left-hand setup to `OperatorEmitter` so legacy parsing now passes through allocator/CFG-aware helpers instead of bespoke branch/slot manipulation.【F:src/fluid/luajit-2.1/src/parser/parse_operators.cpp†L165-L206】 |
| Plain assignments | `IrEmitter::emit_plain_assignment` | Uses `bcemit_store`, `lex_state.assign_adjust`, and now a `RegisterGuard`-managed reset instead of allocator/LValue-driven storage semantics; manual storage flow still needs refactoring.【F:src/fluid/luajit-2.1/src/parser/ir_emitter.cpp†L1022-L1081】 |
