...
5. Exercise the new tracing/pipeline configuration hooks by wiring parser-level self-tests (or debug commands) that run with tracing enabled and capture AST success/failure counts before pursuing the eventual dual-pipeline bytecode diff harness.
