...
## Next Steps
1. Extend the newly added AST/IR scaffolding beyond identifier + field/presence chains to cover additional expression forms (calls, indexing, literals) so parsing can remain independent from `FuncState` mutations.
...
4. Expand the `RegisterAllocator`/`ControlFlowGraph` coverage beyond the assignment helpers so register pressure, jump patching, and `ExpDesc` lifetimes are consistently mediated through the new abstractions before moving into the operator re-write.
5. Broaden the new `jit.parser_trace`/`jit.parser_selftest` harness to cover additional expression and statement forms so the eventual dual-pipeline bytecode diff harness can compare full-program traces rather than only the `local`/identifier pilot cases.
