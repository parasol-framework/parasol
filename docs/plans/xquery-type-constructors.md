...
* Namespace-sensitive constructors outside the built-in schema namespace plus duration/QName handling still depend on the shared parsing utilities (Phase 4). These remain outstanding along with the new test suites from Phase 6.
...
* Phase 6 Fluid regression coverage is now in place: `test_type_constructors.fluid` exercises happy paths, node atomisation, namespace-sensitive behaviour (falling back to XPST0051 assertions until QName/duration constructors land), and lexical/arity error conditions, while `test_type_expr.fluid` gained chaining scenarios that ensure constructor metadata survives `cast as`/`treat as` expressions. Integration of QT3 3.0 constructor groups remains deferred per the latest instructions.
