...
1. **Lifetime management** – Storing `PrologState` on `extXML` conflicts with the current API. `xp::Compile()` accepts a nullable `objXML*` and produces ASTs that are documented as reusable across documents. Tying the compiled prolog to a single document instance would break those expectations and leave `xp::Compile(nullptr, …)` without a storage target.【F:src/xpath/xpath.cpp†L231-L283】
...
Introduce a dedicated descriptor that lives beside the AST:
...
Extend `xpath.fdl` so that `XPathNode` gains an optional shared pointer to `XQueryProlog` plus helpers:
...
Because the structure is generated, this change happens in `xpath.fdl` and will be reflected in the regenerated header. A shared pointer keeps the metadata alive even when multiple `XPathNode` copies or slices exist (e.g., when function bodies are cloned during parsing).
...
2. Introduce `struct ParseResult { std::unique_ptr<XPathNode> expression; std::shared_ptr<XQueryProlog> prolog; };` and change `XPathParser::parse()` to return this result. When no prolog is present `prolog` remains `nullptr` and existing callers can continue to work after a small call-site adjustment.
...
4. Make name resolution consistent by recording namespace bindings in `XQueryProlog` as soon as they are parsed. The parser must apply `default function namespace` when constructing function QNames so that lookup can rely on canonical signatures.
...
1. Add `std::shared_ptr<XQueryProlog> prolog;` to `XPathContext`. The constructor in `xpath_functions.h` should accept an extra optional argument so that evaluators can share the metadata with the function library.【F:src/xpath/api/xpath_functions.h†L43-L74】
2. In `XPathEvaluator::XPathEvaluator` set `context.prolog = query_root.get_prolog();` whenever evaluation begins. This requires threading the compiled query pointer into the evaluator entry points (`evaluate_xpath_expression` and `find_tag`) so that the context is initialised before expression dispatch.【F:src/xpath/eval/eval.cpp†L21-L76】【F:src/xpath/eval/eval_context.cpp†L1-L120】
...
4. Implement a helper for variable lookup that first checks the dynamic environment (`context.variables`) and then consults the prolog’s `variables` map for initialised declarations. External variables remain unresolved and should trigger the existing unsupported-path error until a binding API is introduced.
...
1. **Phase 1 – Infrastructure**
   * Introduce `XQueryProlog`, extend `XPathNode`, teach `xp::Compile()` to attach the descriptor, and modify `XPathContext` to store the shared pointer. No prolog declarations are yet parsed; the structure simply travels alongside the AST.
2. **Phase 2 – Parsing & Storage**
   * Add tokeniser and parser support for namespace, variable, and function declarations. Populate `XQueryProlog` and write unit tests that confirm parsed metadata is preserved across compilation.
3. **Phase 3 – Evaluation**
   * Extend the evaluator to resolve variables and functions from the prolog. Start with inline functions and non-external variables; defer module imports, decimal formats, and options to subsequent iterations.
...
- [ ] Create `src/xpath/api/xquery_prolog.h/.cpp` with the data structures and helper methods described above.
- [ ] Extend `xpath.fdl` and regenerate headers so `XPathNode` stores a shared `XQueryProlog` pointer.
- [ ] Update `XPathTokeniser` and `XPathParser` to return `ParseResult` and populate the prolog descriptor.
- [ ] Adjust `xp::Compile()` to attach the parsed prolog to the compiled `XPathNode`.
- [ ] Thread the prolog pointer through `XPathContext` and `XPathEvaluator`.
- [ ] Implement function and variable lookup helpers that consult `XQueryProlog` before falling back to the built-in library.
