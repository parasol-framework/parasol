...
Tiri provides direct access to Kōtuku's compiled modules through the `mod` namespace.  This chapter defines the interfaces for loading modules, calling module functions, working with C/C++ structures, and interpreting error codes.
...
Loads the Kōtuku module identified by `Name` and returns a module interface object.  `Name` is a case-insensitive alphanumeric string with a maximum length of 32 characters.  Passing an invalid name raises an exception with `ERR_Syntax`.
...
The `include` directive loads the interface definitions (constants, structures, and class definitions) for one or more Kōtuku modules.  It accepts a comma-separated list of module names as string arguments:
...
The structure interface allows Tiri scripts to create and manipulate C/C++ structures declared in the Kōtuku API.  Many class and module functions use structures for returning condensed information.  All structure functions are accessed through the `struct` namespace.

==== struct.new()

[.api]`instance = struct.new(Name, [Fields])` +
[.api]`instance = struct.new(Name, Address)`

Creates a new structure instance from a registered definition identified by `Name`.  The definition must have been registered through an `include` directive, a `mod.load()` call, or a prior call to `MAKESTRUCT()`.  Raises an exception if the definition is not found.
...
[.api]`bytes = struct.size(Name)`

Returns the byte size of the structure definition identified by `Name`.  Does not create a structure instance.  Raises an exception if the definition is not found.

[source,tiri]
----
size = struct.size('GradientStop')
----

==== Field Access

Fields are read and written with dot notation.  Field lookup is case-insensitive.  Reading an unrecognised field name raises an exception.
...
Embedded structures are accessed by chaining dot notation:
