...
Kōtuku is an event-driven framework.  Programs interact with the system by entering a processing loop that dispatches messages, fires timers, and monitors signals.  This chapter defines the `processing` interface, the event subsystem, and the available garbage collector controls.
...
`WakeOnSignal` controls whether a signal (see <<sec-signals>>) can break the sleep early.  Defaults to `true` but is ignored if `Seconds` is `0`.
...
The garbage collector runs a full collection cycle on entry to `processing.sleep()` before the runtime begins waiting for events.  If processing is called in an inner loop, disabling the garbage collector may be desirable if allocated resources remain static.

🔥Encapsulating processing.sleep() in a `try` statement will promote errors to exceptions.
...
Signals provide a lightweight synchronisation mechanism for waking a sleeping script.  A signal is a boolean flag on a Kōtuku object that transitions from _cleared_ to _raised_ when the object's `Signal` action is invoked.  There are no limitations on the class of object that can be used for signalling.
...
[source,tiri]
----
objects = array<object> { obj.new('time', { }), obj.new('time', { }) }
...
script.setKey('Files', files:join(',')) <2>
...
proc = processing.new({ timeout = 10.0, signals = files })
...
Reliable timer subscriptions are available through the system core, `mSys`.  Timers fire callbacks at regular intervals during the processing loop.
...
TIP: Timing calculations take into account the amount of time spent in the callback.  If the interval is 1 second and the callback takes 400ms to execute, the next execution cycle will be in 600ms.
...
A timer can cancel itself by calling `raise ERR_Terminate` from the callback (the exception will be handled without impacting the script):
...
For simple periodic work that does not require a dedicated callback, use a loop around `processing.sleep()` with a timeout:
