...
Comparison operators return a boolean value.  The equality operators (`is`, `is not`, `!=`) accept operands of any type.  The ordering operators (`<`, `>`, `\<=`, `>=`) accept two numbers or two strings; comparing other types without a metamethod raises an error.
...
Logical operators use standard falsiness: only `nil` and `false` are false.  All other values -- including `0`, `""`, and empty tables -- are true.
...
Bitwise operators operate on 32-bit integers using two's-complement representation.  Non-integer operands are truncated to 32-bit integers before the operation.  Shift operators mask the shift count to the range 0-31.
...
The `has` operator tests whether bitwise flags are set in an integer value.
...
The pipe operator (`|>`) passes the result of the left-hand expression as arguments to the right-hand function call.
