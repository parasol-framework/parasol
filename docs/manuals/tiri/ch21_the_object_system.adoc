...
Tiri provides full interoperability with the Kōtuku application framework through a native object interface.  This chapter defines the interface for creating, configuring, querying, and destroying Kōtuku objects from Tiri scripts.
...
Returns the MetaClass object for the given `Object`.  The MetaClass provides access to class-level information such as the class name, fields and registered methods.  Full documentation for the MetaClass interface is provided in the Kōtuku API manual.
...
Reads a field value by name.  If the object is inaccessible or the field cannot be read, returns `Default` or `nil`.  Exceptions are never thrown — the `Default` value has priority.
...
Actions and methods are the primary means of interacting with an initialised object.  Actions are contractual operations defined by the framework (common across many classes), while methods are class-specific operations.
...
Kōtuku enforces a strict parent-child hierarchy.  Every object has exactly one parent and may have any number of children.  By default, `obj.new()` creates objects owned by the script.
...
Object interfaces hold either a strong or weak reference to the underlying Kōtuku object.  The reference type determines when the object is eligible for destruction.
...
The `with` statement locks one or more Kōtuku objects for the duration of a block.  Objects are automatically unlocked when the scope exits through any path: normal completion, `return`, `break`, `continue`, or exception unwinding.
