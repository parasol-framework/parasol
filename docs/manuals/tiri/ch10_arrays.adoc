...
TIP: Direction iteration is preferred over the use of `ipairs()` and `pairs()`, which are supported to maintain seamless compatibility with tables.
...
Resets the array length to zero.  Storage capacity is preserved for reuse.  After clearing, `push()` reuses the existing allocation.
...
arr:clear()
#arr             -- 0
arr:push(100)    -- reuses existing capacity
----
...
All functional methods accept a callback function that receives `(Value, Index)` as arguments.  The `Index` parameter is optional; callbacks that only need the value can omit it.
