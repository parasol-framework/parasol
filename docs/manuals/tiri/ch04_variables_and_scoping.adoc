...
TIP: Upvalues are a powerful tool but also carry a performance penalty when referencing local variables.  A local that is referenced as an upvalue has performance equivalent to a globally scoped variable.
