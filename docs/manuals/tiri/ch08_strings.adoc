...
[[sec-joining-and-splitting]]
=== Joining & Splitting

[[sec-string-join]]
==== string.join()

[.api]`str = string.join(Table, [Separator])`

Joins the contents of `Table` into a single string.  The first argument must be a table; arrays are not accepted.  If `Separator` is provided, it is inserted between each element.  String and number values in the table are included; `nil` values are skipped.
