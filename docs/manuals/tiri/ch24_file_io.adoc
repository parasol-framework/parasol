...
[.api]`data1, data2, ... = io.read([Format, ...])`
...
Reads the entire contents of the file at `Path` and returns it as a string.  Opens the file, reads all data, and closes it in a single operation.  Throws an exception if the file cannot be opened or read.  Returns an empty string if the file is empty.
...
[.api]`data1, data2, ... = file:read([Format, ...])`
...
|`path`
|read/set
|The full virtual path of the file (e.g. `temp:output.txt`).
...
The `mtWatch()` method monitors a file or folder for filesystem events.  Callbacks are dispatched during `processing.sleep()`.
...
Return `ERR_Terminate` from the callback to cancel the watch.
...
|`contentFilter`
|string
|Include files that contain this content.  Searches up to 4096 bytes at a time, sliding through the file.
...
|`driveN:`
|Additional mounted filesystems (`drive2:`, `drive3:`, ...).

|`cdN:`
|CD-ROM drives (`cd1:`, `cd2:`, ...).
|===

**Windows:**

Drive letters (`C:`, `D:`, etc.) are accessible as volumes.  Additional device volumes are enumerated at startup: `usbN:` for USB drives, `cdN:` for optical drives, `portN:` for removable media, and `netN:` for network drives.
...
Volumes can reference other volumes.  The system recursively resolves volume references up to a maximum depth of 10 levels:
...
-- Reading user:settings.tiri checks ~/.kotuku/ first,
-- then falls back to config:users/default/
----
