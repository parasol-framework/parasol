...
namespace dp {
   ...
   template <typename FunctionType = details::default_function_type, typename ThreadType = std::jthread>
      requires std::invocable<FunctionType> and std::is_same_v<void, std::invoke_result_t<FunctionType>>
   class thread_pool {
      public:
         ...
         explicit thread_pool(
            ...
            InitializationFunction init = [](std::size_t) {}) : tasks_(number_of_threads) {
      ...
      template <typename Function> void enqueue_task(Function &&f) {
         ...
         if (prev_in_flight == 0) threads_complete_signal_.store(false, std::memory_order_release);
